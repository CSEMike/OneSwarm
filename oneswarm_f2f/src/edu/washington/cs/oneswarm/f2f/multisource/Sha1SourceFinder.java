...
public class Sha1SourceFinder {
	...
	private boolean isHashActive(byte[] hash) {
		return AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManager(new HashWrapper(hash)) != null;
	}
	...
	private static void recursiveRemoveEmptyDirectory(String canonicalpreefix, File rootDir) throws IOException {
	...
	private void checkForUnusedAutoAddedAndDeleteThem(boolean pauseEncouraged) {
		...
		dm_loop: for (DownloadManager dm : dms) {
			...
			if (dm.getDownloadState().getBooleanAttribute(ONESWARM_AUTO_ADDED)) {
				HashWrapper[] hashesFromDownload = Sha1DownloadManager.getHashesFromDownload(dm, FileListFile.KEY_SHA1_HASH, false);
				...
				boolean properState = dm.getState() == DownloadManager.STATE_DOWNLOADING || dm.getState() == DownloadManager.STATE_SEEDING;
					...
					if (peerManager != null) {
						...
						good_standing_loop: for (Object o : peers) {
							if (o instanceof Sha1Peer) {
								...
								DownloadManager srcDownloadManager = localPeer.getSourceDownloadManager();
								// only non auto added downloads count
								if (srcDownloadManager.getDownloadState().getBooleanAttribute(ONESWARM_AUTO_ADDED)) {
									continue;
								}
				...
				try {
					logger.finer(dm.getDisplayName() + " is no longer needed, removing: properState=" + properState + " virtualPeers=" + hasProperVirtualPeers + " active=" + active + " insterestingSha1=" + interestingSha1Files);
					...
					AzureusCoreImpl.getSingleton().getGlobalManager().removeDownloadManager(dm, true, false);
					dmRemoved = true;

					File torrentFile = new File(Sha1DownloadManager.getMultiTorrentDownloadDir(), id + ".torrent");
					...
					if (simple) {
						...
						if (parent == null || !parent.getName().equals(id)) {
							logger.warning("tried to delete files from simple torrent but the directory has changed! '" + parent + "'");
							continue dm_loop;
						}
						if (toDelete.isFile()) {
						...
						if (grandParent == null || !grandParent.getName().equals(id)) {
							logger.warning("tried to delete files from torrent but the directory has changed! grandparent='" + parent + "'");
							continue dm_loop;
						}
						...
						if (grandParent != null && grandParent.isDirectory() && grandParent.list().length == 0) {
				...
				if (dm.getDownloadState().getBooleanAttribute(ONESWARM_AUTO_ADDED)) {
					...
					for (int i = 0; i < hashesFromDownload.length; i++) {
						...
						if (!file.isSkipped() && !interestingSha1s.contains(hash)) {
							...
							logger.finest("found individual file that no longer is interesting, considering for deletion: " + f);
							...
							if (pauseEncouraged || dm.isPaused()) {
								...
								if (parent == null || !parent.getName().equals(id)) {
									logger.warning("tried to compact files from torrent but the directory has changed! parent='" + parent + "'");
									continue;
								}
	...
	private void addDownload(String originalTorrentName, byte[] metainfo) {
		...
		try {
			...
			TOTorrent torrent = TorrentUtils.readFromBEncodedInputStream(new ByteArrayInputStream(metainfo));
			logger.fine("auto adding download, torrent=" + originalTorrentName + " added=" + new String(torrent.getName()));
			...
			File f = new File(Sha1DownloadManager.getMultiTorrentDownloadDir(), torrentHashString + ".torrent");
			...
			DownloadManager dm = ShareManagerTools.addDownload(selectedFiles, permissions, downloadDir.getCanonicalPath(), true, f, torrent);
			synchronized (Sha1SourceFinder.this) {
			...
			HashWrapper[] hashes = Sha1DownloadManager.getHashesFromDownload(dm, FileListFile.KEY_SHA1_HASH, false);
			if (hashes == null || hashes.length != torrent.getFiles().length) {
				...
				AzureusCoreImpl.getSingleton().getGlobalManager().removeDownloadManager(dm, true, true);
				return;
			}
	...
	private void searchExistingAutoAddedDownloads() {
		...
		logger.finer("using " + (totalDiskSpaceUsed / 1024 / 1024) + " MB for autoadded downloads already");
		...
		// allocated it all...)
		double maxRatioOfFreeSpaceToUse = COConfigurationManager.getFloatParameter("oneswarm.max.multi.torrent.auto.disk.space");
		for (DownloadManager dm : dms) {
			if (dm.getDownloadState().getBooleanAttribute(ONESWARM_AUTO_ADDED)) {
				try {
					File saveLocation = dm.getAbsoluteSaveLocation().getCanonicalFile().getParentFile();
					...
					if (freeSpace == 0) {
						...
						if (freeSpace == 0) {
							...
							freeSpace = 1024 * FileSystemUtils.freeSpaceKb(saveLocation.getCanonicalPath());
						}
					}
					logger.finest("free space at " + saveLocation.getAbsolutePath() + " " + (freeSpace / 1024 / 1024) + "MB");
					...
					for (int i = 0; i < hashesFromDownload.length; i++) {
						...
						if (file.isSkipped() && interestingSha1s.contains(hash)) {
							...
							logger.finer("considering file: " + file.getTorrentFile().getRelativePath());
							...
							if (used > limit) {
								logger.finer("not enabling download of file, free=" + (freeSpace / 1024 / 1024) + " used=" + (used / 1024 / 1024) + " limit=" + (limit / 1024 / 1024));
								continue;
							}
							logger.finer("enabling download of file: " + file.getTorrentFile().getRelativePath());
	...
	private HashSet<HashWrapper> getInterestingSha1s() {
		...
		List<DownloadManager> dms_ = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManagers();

		for (DownloadManager dm : dms_) {
			if (dm.getState() == DownloadManager.STATE_DOWNLOADING && !dm.isDownloadComplete(false) && !dm.getDownloadState().getBooleanAttribute(ONESWARM_AUTO_ADDED)) {
				HashWrapper[] hashesFromDownload = Sha1DownloadManager.getHashesFromDownload(dm, FileListFile.KEY_SHA1_HASH, false);
				DiskManagerFileInfo[] files = dm.getDiskManagerFileInfo();
				for (int i = 0; i < hashesFromDownload.length; i++) {
					DiskManagerFileInfo file = files[i];
	...
	private class AdditionalSourceFinder {
		...
		public void searchForAdditionalSources() {
			...
			for (int i = 0; i < sha1s.length; i++) {
				...
				for (int j = file.getFirstPieceNumber() + 1; j < file.getLastPieceNumber() - 1 && j < pieces.length; j++) {
					if (pieces[j].isDone()) {
						continue;
					}
				...
				final String torrentFileHash = dm.getDisplayName() + ": " + file.getTorrentFile().getRelativePath() + " (" + sha1 + ")";
				...
				searchManager.sendTextSearch("sha1;" + sha1, new TextSearchListener() {
					public void searchResponseReceived(TextSearchResponseItem r) {
						...
						for (FileCollection swarm : swarms) {
							...
							if (AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManager(hw) != null) {
								logger.finest(torrentFileHash + ": already added");
								continue;
							}
							Long lastAttempt = alreadyDownloading.get(hw);
							if (lastAttempt != null && System.currentTimeMillis() - lastAttempt.longValue() < MAX_METAINFO_REQUEST_RATE) {
							...
							logger.finer(torrentFileHash + ": downloading metainfo, swarm=" + infoHash);
							overlayManager.sendMetaInfoRequest(r.getConnectionId(), r.getChannelId(), hw.getBytes(), 0, new PluginCallback<byte[]>() {
								public void requestCompleted(byte[] data) {
									logger.finer(torrentFileHash + ": metainfo download completed, swarm=" + infoHash);
								...
								public void progressUpdate(int progress) {
									logger.finest(torrentFileHash + ": downloading metainfo, swarm=" + infoHash + " progress=" + progress);
								}
