...
public class InvitationConnection {
	 ...
	 * @param _remoteFriend
	 */
	public InvitationConnection(ConnectionEndpoint remoteFriendAddr, final FriendInvitation invitation, AuthCallback callback) {
		...
		this.connection = NetworkManager.getSingleton().createConnection(remoteFriendAddr, new OSF2FAuthMessageEncoder(), new OSF2FAuthMessageDecoder(), true, false, sharedSecret);
			...
			public void connectSuccess(ByteBuffer remaining_initial_data) {
				...
				if (redeeming) {
					...
					if (!invitation.pubKeyMatch(remoteKey)) {
						...
						close("connected to the wrong key: remote public key=" + Base64.encode(remoteKey));
						return;
					}
	...
	private void connectionException(Throwable error) {
		Log.log(LogEvent.LT_WARNING, "got exception in " + "OS Auth session (" + connection + "/" + getRemoteIp() + ") disconnecting: " + error.getMessage() + " (from: " + error.toString() + ")");
	 ...
	 * @param _remoteFriend
	 */
	public InvitationConnection(byte[] remoteKey, NetworkConnection _connection, AuthCallback callback) {
		...
		connection.connect(true, new ConnectionListener() {
			...
			public void connectSuccess(ByteBuffer remaining_initial_data) {
				 ...
				 */
				FriendInvitation i = InvitationConnection.this.callback.getInvitationFromPublicKey(InvitationConnection.this.remoteKey);
				if (i != null) {
					redeeming = true;
	...
	private void handleIncomingHandshake(OSF2FAuthHandshake authHandshake) {
		...
		if (redeeming) {
			 ...
			 * request the key
			 */
			this.sendMessage(new OSF2FAuthRequest(OSF2FAuthMessage.CURRENT_VERSION, OSF2FAuthRequest.AuthType.KEY, null));
	...
	public void handleAuthRequest(OSF2FAuthRequest message) {
		...
		case KEY:
			...
			sendMessage(new OSF2FAuthResponse(OSF2FAuthMessage.CURRENT_VERSION, AuthType.KEY, invitation.getKey()));
	...
	private void updateInvitingStatus(ProtocolStateInviting newStatus) {
		logger.finer("updating status: type=" + connectionType.name() + " old_status=" + protocolStateInviting.name() + " new_status=" + newStatus.name());
		protocolStateInviting = newStatus;
	}

	private void updateRedeemingStatus(ProtocolStateRedeeming newStatus) {
		logger.finer("updating status: type=" + connectionType.name() + " old_status=" + protocolStateRedeeming.name() + " new_status=" + newStatus.name());
	...
	private void handleAuthResponse(OSF2FAuthResponse message) {
		...
		case KEY:
			...
			case FriendInvitation.SECURITY_LEVEL_LOW:
				authenticated();
				sendMessage(new OSF2FAuthStatus(OSF2FAuthMessage.CURRENT_VERSION, OSF2FAuthStatus.STATUS_INVITE_KEY_OK));
	...
	private class IncomingQueueListener implements MessageQueueListener {
		...
		public boolean messageReceived(Message message) {
			packetNum++;
			lastMessageRecvTime = System.currentTimeMillis();
			logger.finer(" got message: " + message.getDescription() + "\t::" + InvitationConnection.this);
