...
public class ServerPublicInfo implements Comparable<ServerPublicInfo>{
	...
	public ServerPublicInfo(String nickname, String ip, String id, int advertBandwidth, String[] exitPolicy, Date lastOutage, String version) {
	 ...
	 * 
	 * The format is:
	 * [reject/accept] [ip range]:[port]
	 * with one policy per line of the string.
	 * 
	 * EX:
	 * reject 66.146.193.31:*
	 * accept *:80
	 * 
	 * @param policy Tor style exit policy array
	 */
	...
	private enum PolicyValue{ACCEPT,REJECT}
	
	private enum NodeType{DOMAIN,PORT}
	...
	private class PolicyTree{
		...
		public void addPolicys(String[] policyStrings){
		...
		public void addPolicy(String policyString){
			...
			if(policyParts[0].equalsIgnoreCase("accept"))
				...
				throw new IllegalArgumentException("Improper Format - First word is not [accept/reject]");
			
			//Deal with port int
			int port = policyParts[2].equals("*") ? -1 : Integer.parseInt(policyParts[2]);
			if(port < -1 || port > 65535)
				throw new IllegalArgumentException("Improper Format - Invalid port");
			
			String[] urlParts = policyParts[1].split("\\.");
			root = addPolicy(urlParts, urlParts.length - 1, port, policy, root);
		}
		
		private PolicyNode addPolicy(String[] url, int index, int port, PolicyValue policy, PolicyNode root){
			if(index < 0){
	...
	class PolicyNode{
		/*
		 * Either domainPart or port will be filled for each node.
		 *	"*" is wild card for domainPart, "" is unused field
		 *	-1 means wild card for port, -2 is unused field
		 */
		String domain;
		int port;
