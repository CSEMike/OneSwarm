...
public class OSF2FPlugin implements Plugin {
    ...
    class OneSwarmURIHandlerListener implements MagnetURIHandlerListener {
        ...
        public byte[] download(final MagnetURIHandlerProgressListener progress, final byte[] hash,
            ...
            logger.fine("Got download request for: " + ByteFormatter.encodeString(hash)
                    + " timeout: " + timeout + " sources has: " + sources.length);
            ...
            try {
                 ...
                 * connected friend
                 */
                while (getOnlineFriends().size() == 0
                        && started + timeout > System.currentTimeMillis()) {
                    logger.fine("Waiting for friend connection before issuing magnet search: "
                ...
                int search_id = OSF2FPlugin.this.sendTextSearch("id:"
                ...
                while (started + timeout > System.currentTimeMillis()) {
                    List<TextSearchResult> results = OSF2FPlugin.this
                            .getTextSearchResult(search_id);
    ...
    public void initialize(final PluginInterface pluginInterface) throws PluginException {
        ...
        MagnetURIHandler.getSingleton()
                .addListener(new OneSwarmURIHandlerListener(pluginInterface));
    }
    ...
    public void sendFileListRequest(int connectionId, long maxCacheAge,
    ...
    public void sendMetaInfoRequest(int connectionId, int channelId, byte[] infohash,
            int lengthHint, PluginCallback<byte[]> callback) {
        main.getOverlayManager().sendMetaInfoRequest(connectionId, channelId, infohash, lengthHint,
                callback);
        // callback.requestCompleted(new byte[10]);
    }
                ...
                if (!forwards.containsKey(channelId)) {
                    ...
                    if (rate > 100 || total > 1024 * 1024) {
                        ...
                        if (sourceMessage instanceof OSF2FTextSearch) {
                            stats.put("content",
                                    ((OSF2FTextSearch) sourceMessage).getSearchString());
                        } else if (sourceMessage instanceof OSF2FHashSearch) {
                            FileListManager filelistManager = main.getOverlayManager()
                                    .getFilelistManager();
                            String torrentNameIfKnown = filelistManager
                                    .getTorrentNameFromInfoHashHash(((OSF2FHashSearch) sourceMessage)
                                            .getInfohashhash());
                        ...
                        if (forward1.isSearcherSide()) {
                            ...
                            stats.put("uid", "fwd" + forward1.getRemoteFriend().getPublicKey()
                                    + "->" + forward2.getRemoteFriend().getPublicKey());
                        } else {
                            stats.put("to", forward1.getRemoteFriend().getNick());
                            stats.put("from", forward2.getRemoteFriend().getNick());

                            stats.put("uid", "fwd" + forward2.getRemoteFriend().getPublicKey()
                                    + "->" + forward1.getRemoteFriend().getPublicKey());
                        }
                        map.add(stats);
                    }
                }
    ...
    public List<Friend> getNewUsersFromXMPP(String xmppNetworkName, String username,
            char[] password, String machineName) throws Exception {

        List<byte[]> knownKeys = main.getFriendManager().getKnownKeysForFriendImport();
        List<PublicKeyFriend> importedFriends = FriendImportManager.importXMPPFriends(knownKeys,
                xmppNetworkName, username, password, machineName);
        if (importedFriends == null) {
            return new LinkedList<Friend>();
        }

        List<Friend> newFriends = main.getFriendManager().convertToFriendArrayAndFilter(
                importedFriends);
        return newFriends;
    ...
    public Map<String, Integer> getNewFriendsCountsFromAutoCheck() {
        Map<String, Integer> newFriendsCountsFromAutoCheck = main.getFriendManager()
        ...
        for (Friend lanUser : lanUsers) {
            if (main.getFriendManager().hasTriedToConnect(lanUser)) {
                if (!main.getFriendManager().isOnIgnoreRequestList(
                        new String(Base64.encode(lanUser.getPublicKey())))) {
                    num++;
                } else {
    ...
    public String getDebugInfo() {
        ...
        long msSinceCheck = System.currentTimeMillis()
                - main.getOverlayManager().getLastConnectionCheckRun();
        ...
        for (FriendConnection f : friendConnections) {
            ...
            b.append(f.getRemoteFriend().getNick() + " (" + f.getRemoteIp() + ") sendQueueBytes="
                    + f.getTotalOutgoingQueueLengthBytes() + " lastSent="
                    + f.getLastMessageSentTime() + " lastReci=" + f.getLastMessageRecvTime() + " "
                    + "imageMetaQueue=" + f.getImageMetaInfoQueueSize() + " " + "torrentMetaQeue="
                    + f.getTorrentMetaInfoQueueSize() + "\n");
        ...
        for (FriendConnection f : friendConnections) {
            ...
            for (OverlayForward of : overlayForwards.values()) {
                b.append("      channel=" + Integer.toHexString(of.getChannelId()) + " "
        ...
        List<String> forwardedSearches = main.getOverlayManager().getSearchManager()
                .debugForwardedSearches();
        ...
        List<String> canceledSearches = main.getOverlayManager().getSearchManager()
                .debugCanceledSearches();
    ...
    public String getSearchDebugLog() {
        ...
        b.append("search cache: \n   total_searches=" + searchesTotal + "   cache_hits="
                + searchCacheHits + "(" + percent + "%)\n\n");
    ...
    public Friend[] scanXMLForFriends(String xml) {
        try {
            XMLDecoder decoder = new XMLDecoder(new BufferedInputStream(new ByteArrayInputStream(
                    xml.getBytes())), this, null, main.getClassLoader());
