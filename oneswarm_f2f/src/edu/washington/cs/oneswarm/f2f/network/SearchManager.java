...
//	static final int SEARCH_DELAY = COConfigurationManager.getIntParameter("f2f_search_forward_delay");
	...
	public SearchManager(OverlayManager overlayManager, FileListManager filelistManager, RandomnessManager randomnessManager, GlobalManagerStats stats) {
		this.stats = stats;
		...
		COConfigurationManager.addAndFireParameterListeners(new String[] { "LAN Speed Enabled", "Max Upload Speed KBs", "oneswarm.search.filter.keywords",
			...
			public void parameterChanged(String parameterName) {
				...
				StringList keywords = COConfigurationManager.getStringListParameter("oneswarm.search.filter.keywords");
				if (keywords != null) {
	...
	public void clearTimedOutSearches() {
		...
		try {
			...
			for (Iterator<ForwardedSearch> iterator = forwardedSearches.values().iterator(); iterator.hasNext();) {
			...
			for (Iterator<SentSearch> iterator = sentSearches.values().iterator(); iterator.hasNext();) {
	...
	public List<String> debugCanceledSearches() {
		...
		try {
			for (Integer s : canceledSearches.keySet()) {
				l.add("search=" + Integer.toHexString(s) + " age=" + ((System.currentTimeMillis() - canceledSearches.get(s)) / 1000) + "s");
			}
		} finally {
			lock.unlock();
		}
	...
	public List<String> debugForwardedSearches() {
		...
		try {
			for (ForwardedSearch f : forwardedSearches.values()) {
				l.add("search=" + Integer.toHexString(f.getSearchId()) + " responses=" + f.getResponseNum() + " age=" + (f.getAge() / 1000) + "s");
			}
		} finally {
			lock.unlock();
		}
	...
	public List<String> debugSentSearches() {
		...
		try {
			for (SentSearch s : sentSearches.values()) {
				l.add("search=" + Integer.toHexString(s.getSearch().getSearchID()) + " responses=" + s.getResponseNum() + " age=" + (s.getAge() / 1000) + "s");
			}
		} finally {
			lock.unlock();
		}
	...
	private double getAverageUploadPerRunningTorrent() {
		...
		final List<DownloadManager> downloadManagers = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManagers();
	...
	private boolean handleHashSearch(final FriendConnection source, final OSF2FHashSearch msg) {
		...
		if (infohash != null) {
			DownloadManager dm = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManager(new HashWrapper(infohash));
			...
			if (dm != null) {
				...
				boolean allowed = OverlayTransport.checkOSF2FAllowed(dm.getDownloadState().getPeerSources(), dm.getDownloadState().getNetworks());
				if (!allowed) {
				...
				final int overlayDelay = overlayManager.getLatencyDelayForInfohash(source.getRemoteFriend(), infohash);
					...
					public void run() {
						try {
							...
							if (!isSearchCanceled(msg.getSearchID())) {
								final OSF2FHashSearchResp response = new OSF2FHashSearchResp(OSF2FMessage.CURRENT_VERSION, msg.getSearchID(), newChannelId, pathID);

								final OverlayTransport transp = new OverlayTransport(source, newChannelId, infohash, transportFakePathId, false, overlayDelay);
							...
							Debug.out("got an error when registering incoming transport to '" + source.getRemoteFriend().getNick() + "': " + e.message);
						}
					}
	...
	private void handleIncomingHashSearchResponse(OSF2FHashSearch hashSearch, FriendConnection source, OSF2FHashSearchResp msg) {
		...
		if (infoHash == null) {
			logger.warning("got channel setup request, " + "but the infohash we searched for " + "is not in filelistmananger");
			return;
		}
		...
		if (source.hasRegisteredPath(msg.getPathID())) {
			logger.finer("got channel setup response, " + "but path is already used: sending back a reset");
			source.sendChannelRst(new OSF2FChannelReset(OSF2FMessage.CURRENT_VERSION, msg.getChannelID()));
			return;
		}

		OverlayTransport overlayTransport = new OverlayTransport(source, msg.getChannelID(), infoHash, msg.getPathID(), true, overlayManager.getLatencyDelayForInfohash(source.getRemoteFriend(), infoHash));
		// register it with the friendConnection
		try {
	...
	public void handleIncomingSearch(FriendConnection source, OSF2FSearch msg) {
		...
		try {
			...
			if (forwardedSearches.containsKey(msg.getSearchID()) || sentSearches.containsKey(msg.getSearchID()) || delayedSearchQueue.isQueued(msg)) {
				return;
			}
		} finally {
			lock.unlock();
		...
		if (msg instanceof OSF2FHashSearch) {
			...
			logger.warning("received unrecgonized search type: " + msg.getID() + " / " + msg.getClass().getCanonicalName());
		}
	...
	public void handleIncomingSearchResponse(FriendConnection source, OSF2FSearchResp msg) {
		...
		if (sentSearch != null) {
			...
			if (sentSearch.getResponseNum() > mMaxSearchResponsesBeforeCancel) {
				...
				if (sendCancel) {
					overlayManager.sendSearchOrCancel(new OSF2FSearchCancel(OSF2FMessage.CURRENT_VERSION, msg.getSearchID()), true, false);
				}
			}
			if (search instanceof OSF2FHashSearch) {
				// ok, it was a hash search that we sent
				handleIncomingHashSearchResponse((OSF2FHashSearch) search, source, (OSF2FHashSearchResp) msg);
				...
				try {
					...
					textSearchManager.gotSearchResponse(search.getSearchID(), source.getRemoteFriend(), fileList, textSearchResp.getChannelID(), source.hashCode());
		...
		// sentsearch == null
			...
			try {
				...
				if (search == null) {
					logger.warning("got response for unknown search:" + source + ":" + msg.getDescription());
					return;
				}
			...
			if (search.getResponseNum() > mMaxSearchResponsesBeforeCancel) {
				...
				logger.finest("Sending cancel for someone elses search!, searcher=" + searcher.getRemoteFriend() + " responder=" + responder.getRemoteFriend() + ":\t" + search);
				overlayManager.forwardSearchOrCancel(source, new OSF2FSearchCancel(OSF2FMessage.CURRENT_VERSION, msg.getSearchID()));
			} else {
				search.gotResponse();
				// register the forwarding
				logger.finest("registering overlay forward: " + searcher.getRemoteFriend().getNick() + "<->" + responder.getRemoteFriend().getNick());
				try {
					...
					String direction = "'" + responder.getRemoteFriend().getNick() + "'->'" + searcher.getRemoteFriend().getNick() + "'";
		...
		if (filteredKeywords.length > 0) {
			...
			for (String filter : filteredKeywords) {
				if (searchString.contains(filter)) {
					logger.fine("Blocking search due to filter: " + searchString + " matched by: " + filter);
					return false;
				}
			}
		}

		List<FileCollection> results = filelistManager.handleSearch(source.getRemoteFriend(), searchString);
	...
	public void sendDirectedHashSearch(FriendConnection target, byte[] infoHash) {
		...
		OSF2FHashSearch search = new OSF2FHashSearch(OSF2FMessage.CURRENT_VERSION, newSearchId, metainfohashhash);
		lock.lock();
		try {
	...
	public void sendHashSearch(byte[] infoHash) {
		...
		OSF2FSearch search = new OSF2FHashSearch(OSF2FMessage.CURRENT_VERSION, newSearchId, metainfohashhash);
	...
	private static String handleKeyWords(String searchString) {
		...
		for (String s : split) {
			...
			for (int i = 0; i < interestingKeyWords.length; i++) {
				String fromId = convertToBase64(s, interestingKeyWords[i], interestingKeyWordExectedKeyLen[i]);
				if (fromId != null) {
					toAdd = fromId;
				}
			}
			...
			public void run() {
				while (true) {
					try {
						...
						if (timeUntilSend > 0) {
							logger.finer("got search (" + e.search.getDescription() + ") to forward, waiting " + timeUntilSend + " ms until sending");
	...
	class TextSearchManager {
				...
				for (TextSearchResponseItem item : resps.getItems()) {
					for (FileCollection collection : item.getFileList().getElements()) {
						if (result.containsKey(collection.getUniqueID())) {
							...
							GlobalManager globalManager = AzureusCoreImpl.getSingleton().getGlobalManager();
							DownloadManager dm = globalManager.getDownloadManager(new HashWrapper(collection.getUniqueIdBytes()));
							if (dm == null) {
								alreadyInLibrary = false;
							}
							result.put(collection.getUniqueID(), new TextSearchResult(item, collection, alreadyInLibrary));
						}
					}
				}
		...
		public void gotSearchResponse(int searchId, Friend throughFriend, FileList fileList, int channelId, int connectionId) {
			TextSearchResponse r = responses.get(searchId);
			if (r != null) {
				long age = System.currentTimeMillis() - r.getTime();
				TextSearchResponseItem item = new TextSearchResponseItem(throughFriend, fileList, age, channelId, connectionId);
				r.add(item);
