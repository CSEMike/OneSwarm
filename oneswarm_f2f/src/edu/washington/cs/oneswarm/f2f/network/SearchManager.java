        ...
        if (results.size() > 0) {
            if (canRespondToSearch()) {
                ...
                for (FileCollection c : results) {
                    ...
                    int searchDelay = overlayManager.getSearchDelayForInfohash(
                            source.getRemoteFriend(), c.getUniqueIdBytes());
                    int overlayDelay = overlayManager.getLatencyDelayForInfohash(
                            source.getRemoteFriend(), c.getUniqueIdBytes());

                    delayedExecutionTasks.add(new DelayedExecutionEntry(time + searchDelay
                            + overlayDelay, 0, new TimerTask() {
                        @Override
                        public void run() {
                            /*
                             * check if the search got canceled while we were
