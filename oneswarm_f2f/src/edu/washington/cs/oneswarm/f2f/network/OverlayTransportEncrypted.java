...
public class OverlayTransportEncrypted extends OverlayTransport {
    private final static Logger logger = Logger
            .getLogger(OverlayTransportEncrypted.class.getName());
    ...
    public OverlayTransportEncrypted(FriendConnection connection, int channelId, byte[] infohash,
            int pathID, boolean outgoing, long overlayDelayMs, byte[] key)
    ...
    public long write(ByteBuffer[] buffers, int array_offset, int length) throws IOException {
        ...
        try {
            ...
            if (isReadyForWrite(null)) {
                DirectByteBuffer msgBuffer = DirectByteBufferPool.getBuffer(
                        DirectByteBuffer.AL_MSG,
    ...
    public long read(ByteBuffer[] buffers, int array_offset, int length) throws IOException {
        ...
        for (int i = 0; i < buffers.length; i++) {
            tempBufferPool[i] = DirectByteBufferPool.getBuffer(DirectByteBuffer.AL_MSG,
                    buffers[i].remaining());
