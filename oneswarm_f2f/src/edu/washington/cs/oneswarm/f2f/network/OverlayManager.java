...
public class OverlayManager {
	...
	private int mMIN_DELAY_LINK_LATENCY = COConfigurationManager.getIntParameter("f2f_search_emulate_hops_min") * COConfigurationManager.getIntParameter("f2f_overlay_emulate_link_latency_max");
	...
	public OverlayManager(FriendManager _friendManager, PublicKey _ownPublicKey, FileListManager _fileListManager, GlobalManagerStats _stats) {
		stats = _stats;
	...
	public boolean createIncomingConnection(byte[] publicKey, NetworkConnection netConn) {
		...
		if (isConnectionAllowed(netConn.getEndpoint().getNotionalAddress().getAddress(), publicKey)) {
			...
			new FriendConnection(stats, queueManager, netConn, friend, filelistManager, new FriendConnectionListener());
	...
	public boolean createOutgoingConnection(ConnectionEndpoint remoteFriendAddr, Friend friend) {
		if (isConnectionAllowed(remoteFriendAddr.getNotionalAddress().getAddress(), friend.getPublicKey())) {
			final FriendConnection fc = new FriendConnection(stats, queueManager, remoteFriendAddr, friend, filelistManager, new FriendConnectionListener());
			/*
	...
	private boolean deregisterConnection(FriendConnection connection) {
		lock.lock();
		try {
			Log.log("deregistered connection: " + connection.toString() + " " + connections.containsKey(connection.hashCode()) + " ", logToStdOut);
			...
			if (connectedConn != null && connectedConn.isHandshakeReceived()) {
				...
				if (connectedConn.hashCode() != friendConnId) {
					...
					if (!fileListReceived) {
						Log.log("connection closed, existing connection found, set to handshaking: " + remoteFriend.getNick(), logToStdOut);
						remoteFriend.setStatus(Friend.STATUS_HANDSHAKING);
						remoteFriend.setConnectionId(Friend.NOT_CONNECTED_CONNECTION_ID);
					} else {
						Log.log("connection closed, existing connection found, set to connected: " + remoteFriend.getNick(), logToStdOut);
	...
	void forwardSearchOrCancel(FriendConnection ignoreConn, OSF2FSearch msg) {
		for (FriendConnection conn : connections.values()) {
			if (ignoreConn.hashCode() == conn.hashCode()) {
				Log.log("not forwarding search/cancel to: " + conn + " (source friend)", logToStdOut);
				continue;
			}
		...
		for (int connectionId : connections.keySet()) {
			...
			if (status == Friend.STATUS_OFFLINE && c.isHandshakeReceived()) {
				...
				if (!handshakeCompletedFully) {
					Debug.out("getConnectedFriends, existing connection found, settings to handshaking: " + remoteFriend.getNick());
					remoteFriend.setStatus(Friend.STATUS_HANDSHAKING);
				} else {
					Debug.out("getConnectedFriends, existing connection found, settings to connected: " + remoteFriend.getNick());
	...
	public int getSearchDelayForInfohash(Friend destination, byte[] infohash) {
		if (destination.isCanSeeFileList()) {
			...
			int searchDelay = responseDelayRandomnesManager.getDeterministicNextInt(infohash, mMIN_RESPONSE_DELAY, mMAX_RESPONSE_DELAY);
	...
	public int getLatencyDelayForInfohash(Friend destination, byte[] infohash) {
		if (destination.isCanSeeFileList()) {
			...
			return responseDelayRandomnesManager.getDeterministicNextInt(infohash, mMIN_DELAY_LINK_LATENCY, mMAX_DELAY_LINK_LATENCY);
		}
	}
	...
	public double getTransportDownloadKBps() {
		...
		for (FriendConnection fc : conns) {
			final Map<Integer, OverlayTransport> ot = fc.getOverlayTransports();

			for (OverlayTransport o : ot.values()) {
	...
	public boolean isConnectionAllowed(InetAddress remoteIP, byte[] remotePubKey) {
		...
		if (Arrays.equals(remotePubKey, ownPublicKey.getEncoded()) && remoteIP.equals(myInstance.getExternalAddress())) {
			Log.log(LogEvent.LT_INFORMATION, "connection from self not allowed (if same ip)", logToStdOut);
			return false;
		} else if (friend == null) {
			Log.log(LogEvent.LT_WARNING, " access denied (not friend): " + remoteIP, logToStdOut);
			return false;
		} else if (friend.isBlocked()) {
			Log.log(LogEvent.LT_WARNING, " access denied (friend blocked): " + remoteIP, logToStdOut);
			return false;
		} else if (friend.getFriendBannedUntil() > System.currentTimeMillis()) {
			double minutesLeft = friend.getFriendBannedUntil() - System.currentTimeMillis() / (60 * 1000.0);
			friend.updateConnectionLog(true, "incoming connection denied, friend blocked for " + minutesLeft + " more minutes because of: " + friend.getBannedReason());
			Log.log(LogEvent.LT_WARNING, " access denied (friend blocked for " + minutesLeft + " more minutes): " + remoteIP, logToStdOut);
			return false;
		}

		for (FriendConnection c : connections.values()) {
			if (c.getRemoteFriend().equals(friend)) {
				Log.log(LogEvent.LT_WARNING, " access denied (friend already connected): " + remoteIP, logToStdOut);
				return false;
			}
		}
		Log.log(LogEvent.LT_INFORMATION, "friend connection ok: " + remoteIP + " :: " + friend, logToStdOut);
		return true;
	}
	...
	public boolean sendFileListRequest(int connectionId, long maxCacheAge, PluginCallback<FileList> callback) {
		...
		if (conn != null) {
			...
			if (oldList != null && (System.currentTimeMillis() - oldList.getCreated()) < maxCacheAge) {
			...
			System.err.println("tried to get filelist for unknown connection id (friend just went offline?)!, stack trace is:");
	...
	public boolean sendMetaInfoRequest(int connectionId, int channelId, byte[] infohash, int lengthHint, PluginCallback<byte[]> callback) {
		FriendConnection conn = connections.get(connectionId);
		Log.log("sending metainfo request to " + conn);
		if (conn != null) {
			conn.sendMetaInfoRequest(OSF2FMessage.METAINFO_TYPE_BITTORRENT, channelId, infohash, lengthHint, callback);
			return true;
		}
		return false;
	...
	void sendSearchOrCancel(OSF2FSearch search, boolean skipQueue, boolean forceSend) {
		...
		for (FriendConnection conn : connections.values()) {
			...
			if (search instanceof OSF2FHashSearch) {
				...
				searchTimingsLogger.log(System.currentTimeMillis() + ", send_search, " + conn.getRemoteFriend().getNick() + ", " + hs.getSearchID() + ", " + hs.getInfohashhash());
			}
		}
		/*
	...
	class FriendConnectionListener {
		public boolean connectSuccess(FriendConnection friendConnection) {
			if (!registerConnection(friendConnection)) {
				Log.log("Unable to register connection, " + "connect count to high, closing connection", logToStdOut);
				friendConnection.close();
				return false;
			}
			return true;
		...
		public void gotSearchMessage(FriendConnection friendConnection, OSF2FSearch msg) {
			if (msg instanceof OSF2FHashSearch) {
				...
				searchTimingsLogger.log(System.currentTimeMillis() + ", search, " + friendConnection.getRemoteFriend().getNick() + ", " + hs.getSearchID() + ", " + hs.getInfohashhash() + ", " + friendConnection.getRemoteIp().getHostAddress());
			}
		...
		public void gotSearchResponse(FriendConnection friendConnection, OSF2FSearchResp msg) {
			if (msg instanceof OSF2FHashSearchResp) {
				...
				searchTimingsLogger.log(System.currentTimeMillis() + ", response, " + friendConnection.getRemoteFriend().getNick() + ", " + hsr.getSearchID() + ", " + hsr.getChannelID() + ", " + friendConnection.getRemoteIp().getHostAddress() + ", " + hsr.getPathID());
			}
		...
		public void handshakeCompletedFully(final FriendConnection friendConnection) {
			...
			List<DownloadManager> dms = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManagers();
			...
			FileList remoteFileList = filelistManager.getFriendsList(friendConnection.getRemoteFriend());
			for (byte[] hash : runningDownloadHashes) {
				if (remoteFileList.contains(hash)) {
					Log.log("sending search for '" + Base32.encode(hash) + "' to " + friendConnection.getRemoteFriend(), logToStdOut);
