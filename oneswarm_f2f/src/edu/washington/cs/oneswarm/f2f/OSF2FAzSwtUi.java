...
public class OSF2FAzSwtUi {
    ...
    private void addViaPeerColumn(TableManager tableManager) {
        ...
        column.addCellRefreshListener(new TableCellRefreshListener() {
            public void refresh(TableCell cell) {
                ...
                OverlayTransport tr = (OverlayTransport) peer.getPEPeer().getData(
    ...
    private void addConnectionWeightColumn(TableManager tableManager) {
        ...
        column = tableManager.createColumn(TableManager.TABLE_TORRENT_PEERS,
                COLUMNID_CONNECTION_WEIGHT);
        column.initialize(TableColumn.ALIGN_TRAIL, TableColumn.POSITION_LAST, 50,
                TableColumn.INTERVAL_LIVE);
        column.addCellRefreshListener(new TableCellRefreshListener() {
            public void refresh(TableCell cell) {
    ...
    class ViewListener implements UISWTViewEventListener {
        ...
        private void initialize(Composite parent) {
            ...
            friendConnectionTable.addSelectionListener(new SelectionListener() {
                ...
                public void widgetSelected(SelectionEvent e) {
                    TableItem i = friendConnectionTable.getItem(friendConnectionTable
                            .getSelectionIndex());
                    if (i != null) {
            ...
            friendConnectionTable.addKeyListener(new KeyListener() {
                ...
                public void keyPressed(KeyEvent e) {
                    if (e.keyCode == java.awt.event.KeyEvent.VK_SPACE
                            || e.keyCode == java.awt.event.KeyEvent.VK_BACK_SPACE) {
            ...
            for (columnHeader header : CONN_COLUMN_HEADERS) {
                org.eclipse.swt.widgets.TableColumn col = new org.eclipse.swt.widgets.TableColumn(
                        friendConnectionTable, SWT.LEFT);
            ...
            for (columnHeader header : OVER_COLUMN_HEADERS) {
                org.eclipse.swt.widgets.TableColumn col = new org.eclipse.swt.widgets.TableColumn(
                        overlayConnectionTable, SWT.LEFT);
                col.setText(header.name);
                col.setWidth(header.width);
            }
        ...
        private final columnHeader[] CONN_COLUMN_HEADERS = {
                new columnHeader("connection id", 100), new columnHeader("remote friend", 100),
                new columnHeader("remote ip:port", 150), new columnHeader("upload speed", 150),
                new columnHeader("connection age", 100), new columnHeader("queue (ms)", 100),
                new columnHeader("queue (bytes)", 100), new columnHeader("proto uploaded", 100),
                new columnHeader("proto downloaded", 100), new columnHeader("data uploaded", 100),
                new columnHeader("data downloaded", 100), new columnHeader("forward num", 100),
                new columnHeader("transport num", 100), new columnHeader("last sent", 100),
                new columnHeader("last recv", 100), new columnHeader("avg upload", 100),
                new columnHeader("avg download", 100) };
        ...
        private void addConnections(FriendConnection c) {
            ...
            item.setText(new String[] {
                    "" + c.hashCode(),
                    c.getRemoteFriend().getNick(),
                    c.getRemoteIp().getHostAddress() + ":" + c.getRemotePort(),
                    formatter.formatByteCountToKiBEtcPerSec(c.getCurrentUploadSpeed()),
                    formatter.formatTimeFromSeconds(c.getConnectionAge() / 1000),
                    "not calculated",
                    c.getForwardQueueLengthBytes() + "",
                    formatter.formatByteCountToKiBEtc(c.getProtocolBytesUploaded()),
                    formatter.formatByteCountToKiBEtc(c.getProtocolBytesDownloaded()),
                    formatter.formatByteCountToKiBEtc(c.getDataBytesUploaded()),
                    formatter.formatByteCountToKiBEtc(c.getDataBytesDownloaded()),
                    "" + c.getOverlayForwards().size(),
                    "" + c.getOverlayTransports().size(),
                    formatter.formatTimeFromSeconds(c.getLastMessageSentTime() / 1000),
                    formatter.formatTimeFromSeconds(c.getLastMessageRecvTime() / 1000),
                    formatter.formatByteCountToKiBEtcPerSec((1000 * c.getDataBytesUploaded() + c
                            .getProtocolBytesUploaded()) / c.getConnectionAge()),
                    formatter.formatByteCountToKiBEtcPerSec((1000 * c.getDataBytesDownloaded() + c
                            .getProtocolBytesDownloaded()) / c.getConnectionAge()) });
        ...
        private final columnHeader[] OVER_COLUMN_HEADERS = { new columnHeader("type", 100),
                new columnHeader("id", 100), new columnHeader("friend 1", 100),
                new columnHeader("f1 ip:port", 150), new columnHeader("friend 2", 100),
                new columnHeader("f2 ip:port", 150), new columnHeader("connection age", 100),
                new columnHeader("time since last msg", 150), new columnHeader("bytes in", 100),
                new columnHeader("bytes out", 100), new columnHeader("source message", 200) };
        ...
        private void addOverlays(FriendConnection sel) {
            ...
            for (Integer id : forwards.keySet()) {
                ...
                item.setText(new String[] { "forward", f.getChannelId() + "",
                        ...
                        sel.getRemoteIp().getHostAddress() + ":" + sel.getRemotePort(),
                        f.getRemoteFriend().getNick(), f.getRemoteIpPort(),
                        formatter.formatTimeFromSeconds(f.getAge() / 1000),
                        formatter.formatTimeFromSeconds(f.getLastMsgTime() / 1000),
                        formatter.formatByteCountToKiBEtc(f.getBytesForwarded()),
                        formatter.formatByteCountToKiBEtc(f.getBytesForwarded()),
            ...
            for (Integer id : transports.keySet()) {
                ...
                item.setText(new String[] { "transport", f.getPathID() + "", "Me", "N/A",
                        ...
                        formatter.formatTimeFromSeconds(f.getAge() / 1000),
                        formatter.formatTimeFromSeconds(f.getLastMsgTime() / 1000),
                        formatter.formatByteCountToKiBEtc(f.getBytesIn()),
                        formatter.formatByteCountToKiBEtc(f.getBytesOut()) });
            }
        }
    }
