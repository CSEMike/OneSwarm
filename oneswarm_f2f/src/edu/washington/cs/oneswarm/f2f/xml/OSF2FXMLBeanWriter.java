...
public class OSF2FXMLBeanWriter<T> implements Runnable {
	...
	public OSF2FXMLBeanWriter(T[] objects, Semaphore lock, String filename, boolean makeBackup, boolean allowDecreasedSize) {
	...
	public void run() {
		...
		try {
			if (!OSF2F_DIR.isDirectory()) {
				...
				if (!success) {
					System.err.println("ERROR: failed to create dir: " + OSF2F_DIR.getAbsolutePath());
				}
			}
			...
			if (oldFile.isFile() && makeBackup) {
				...
				File backupFile = new File(oldFile.getCanonicalPath() + ".backup" + nft.format(previousBackupFile + 1));
				logger.finest("moving: " + oldFile.getAbsolutePath() + " to " + backupFile.getAbsolutePath());
			...
			if (oldFile.length() < f.length() + 1000 || allowDecreasedSize) {
				...
				System.err.println("the new version of the friends file is significantly smaller than the old version. Not overwriting. Old: " + oldFile.length() + " New: " + f.length());
			}
	...
	private static void renameOrCopy(File toRenameOrCopy, File dst) throws IOException {
		if (toRenameOrCopy.renameTo(dst) == false) {
			if (FileUtil.copyFile(toRenameOrCopy, dst) == false) {
				throw new IOException("couldn't rename or copy: " + toRenameOrCopy.toString() + " to " + dst.toString());
			}
