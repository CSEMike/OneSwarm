...
public class OSF2FChat implements OSF2FMessage {
	...
	private static final int MAX_LENGTH = 1024 * 2; // 2k of XML ought to be enough for anybody
	...
	public Message deserialize(DirectByteBuffer data, byte version) throws MessageException {
		...
		if( length > MAX_LENGTH ) {
			throw new MessageException("[" + getID() + "] decode error: message length greater than " + MAX_LENGTH + " (" + length + ")");
		}
		...
		try {
			...
			DefaultHandler handler = new DefaultHandler(){
				public void startElement (String uri, String localName,
					...
					if( qName.equals(MESSAGE_ELEMENT) ) {
						plain_text_message = Entities.XML.unescape(attributes.getValue(PLAIN_TEXT_ATTRIB));
					}
				}
			};
			...
			if( plain_text_message == null ) {
				throw new MessageException("[" + getID() + "] decode error: no plain text message after XML parsing");
			}
			
		} catch (SAXException e) {
			throw new MessageException("[" + getID() + "] XML decode error: " + e.getClass().getName() + " / " + e.toString() + " bytes: " + (new String(in)));
		} catch (IOException e) {
			throw new MessageException("[" + getID() + "] XML decode error: " + e.getClass().getName() + " / " + e.toString() + " bytes: " + (new String(in)));
		}
	...
	public DirectByteBuffer[] getData() {
		if (buffer == null) {
			...
			doc.addElement(new XML(MESSAGE_ELEMENT).addXMLAttribute(PLAIN_TEXT_ATTRIB, org.apache.commons.lang.Entities.XML.escape(plain_text_message)));
