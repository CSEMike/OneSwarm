...
public class OSF2FHashSearchResp implements OSF2FMessage, OSF2FSearchResp {
    ...
    public OSF2FHashSearchResp clone() {
        return new OSF2FHashSearchResp(OSF2FMessage.CURRENT_VERSION, this.getSearchID(),
                this.getChannelID(), this.getPathID());
    }
    ...
    public String getDescription() {
        if (description == null) {
            description = OSF2FMessage.ID_OS_CHANNEL_SETUP + "\tsearch="
                    + Integer.toHexString(searchID) + "\tchannel=" + Integer.toHexString(channelID)
                    + "\tpathid=" + Integer.toHexString(originalPathID);
        }
