...
public class SearchTester {
	...
	private final static byte[] infohash = Base64.decode(new String(
			"8KiDKijBtD0Wvtm8dbwmslnagoA="));
	...
	public SearchTester() {
		try {
			...
			ipc.invoke("sendFileListRequest", new Object[] {});
			...
			if (main.getOverlayManager().getFilelistManager().getMetainfoHash(
				...
				for (FriendConnection friendConnection : conn) {
					friendConnection.sendMetaInfoRequest(
							OSF2FMessage.METAINFO_TYPE_BITTORRENT, infohash);
				}
				Thread.sleep(10000);
	...
	private void startDownload() {
		DownloadManager downloadManager = TorrentUtils
				.getDownloadManager(new HashWrapper(infohash));
		conn.get(0).setFriendToFriendOnly(downloadManager);
			...
			downloadManager.addListener(new DownloadManagerListener() {
				...
				public void stateChanged(DownloadManager manager, int state) {
					if (state == DownloadManager.STATE_READY) {
						...
						manager.stopIt(DownloadManager.STATE_STOPPED, true,
								true);
						System.err.println("torrent in error state, "
								+ "removing, try again later");
					}
					Log.log("state=" + state);
