...
class CLI_Main {
	...
	public CLI_Main(String[] args) {
		...
		if (torrent_bytes != null)
			start_distribution(torrent_bytes, keep_curr, outfile, wait_after_finished_secs, wait_after_ratio);
		else {
					...
					if (state == DownloadManager.STATE_DOWNLOADING) {
						...
						(new Timer("Download status reporter", true)).schedule(new TimerTask() {
							public void run() {
								...
								System.out.println(" DL: " + stats.getDataReceiveRate() / 1024 + " [" + stats.getTotalDataBytesReceived() / (1024) + "]" + " UL: " + stats.getDataSendRate() / 1024 + " [" + stats.getTotalDataBytesSent() / (1024) + "]" + " Peers: " + manager.getPeerManager().getPeers().size() + " %: " + (double) ((double) stats.getTotalGoodDataBytesReceived() / (double) manager.getSize()) * 100.0 + " MaxUL: " + COConfigurationManager.getIntParameter("Max Upload Speed KBs"));
							}
						}, 1000, 5 * 1000);
					}
				}
