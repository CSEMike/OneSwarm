...
import edu.uw.cse.netlab.reputation.PerSwarmOneHopUnchoker;
	...
	private void doUnchokes() {  
		...
		if( !UploadSlotManager.AUTO_SLOT_ENABLE ) {		 //manual per-torrent unchoke slot mode
			...
			if( seeding_mode ) {
				...
				if( unchoker == null && this.getHash() == null ) {
					System.err.println("hash is null for PEPeerControlImpl: " + this.getDisplayName() + " ...we're probably stopping" );
					return;
				}
				if( unchoker == null || !(unchoker instanceof PerSwarmOneHopUnchoker) ) {
					//unchoker = new SeedingUnchoker();
					unchoker = new PerSwarmOneHopUnchoker(new HashWrapper(this.getHash()));
				}
			}
			else {
