...
public class MultiPeerUploader implements RateControlledEntity {
	...
	/**
	 * PIAMOD -- adding java standard logging to this. 
	 */
	private static Logger logger = Logger.getLogger(MultiPeerUploader.class.getName());
  ...
  public MultiPeerUploader( RateHandler rate_handler ) {
    ...
    logger.info("OneSwarm MultiPeerUploader created");
    logger.fine("fine log level?");
  }
...
//    	NetworkConnectionBase conn = removeWeightedRandomConnection();
   ...
   * PIAMOD -- 
   * 
   * Azureus uses round-robin scheduling of connection by default, which doesn't allow us 
   * to do fine-grained control of rates. We could do this by setting maximum rates per round 
   * (this is supported by Azureus), but this results in poor utilization if we under (or over) estimate 
   * the bandwidth capacity of a path. 
   * 
   * Instead, we use connection weights that approximate this. These are based on noramlized 
   * reputation scores but enforced here. Since these weights change frequently (and we don't 
   * want to have the unchoker notifying this class of anything), we use rejection sampling 
   * for the weighted selection rather than speedier methods based on sorting the set of connection 
   * objects. 
   * 
   * Rejection sampling is fast enough for us so long as the disparity in reputation values is 
   * not too large. If this happens, many samples are likely to be rejected before accepting a 
   * candidate (driving up CPU usage) and potentially resulting in low-weight, unchoked 
   * connections receiving a vanishingly small amount of capacity. In practice, both of 
   * these are likely to annoy users more than a small lack of fidelity to connection 
   * weights, which is what we use here. By imposing a minimum acceptance probability (5% as of this 
   * writing), we ensure that connections won't starve and some connection will be selected 
   * quickly (limiting CPU). The tradeoff is reduced fidelity to the weight.
   * 
   * We remove the returned connection since the only function that calls this (write()) assumes 
   * this behavior and adds the connection back to the list of ready connections (and we want to 
   * minimize code changes during version syncs).
   * 
   * Note that this assumes the connection weights are normalized 0..1 (the unchoker does this).  
   * 
   */
  Random rand = new Random();
  private NetworkConnectionBase removeWeightedRandomConnection() {
	  /**
	   * An implementation detail here is that ready_connections is a LinkedList (since 
	   * the rest of the class does add / remove operations frequently and array shifting is a pain), 
	   * but here we need random access for the rejection sampling. 
	   * 
	   * I'm not quite sure whether it's faster to do an array conversion here for fast random access (but 
	   * take the conversion hit), not convert and accept lengthy lookups, or accept the array shifting 
	   * that would result from making this an arraylist overall. Hopefully little time will be spent 
	   * here and it won't really matter. 
	   */
	for( int trial=0; trial<200; trial++  ) {
		int index = rand.nextInt(ready_connections.size());
		NetworkConnectionBase candidate = (NetworkConnectionBase) ready_connections.get(index);
		if( candidate instanceof NetworkConnectionImpl ) {
			if( rand.nextDouble() < Math.max(((NetworkConnectionImpl)candidate).getWeight(), 0.05) )
			{
				ready_connections.remove(index);
				return candidate;
			}
		}
		else {
			logger.warning("NetworkConnectionBase that's not a NetworkConnctionImpli in MultiPeerUploader: " + candidate.getClass().getName());
			return candidate;
		}
	}
	logger.warning("No accepted weighted sample after 200 trials, removing first of " + ready_connections.size());
	return (NetworkConnectionBase)ready_connections.removeFirst();
  }
