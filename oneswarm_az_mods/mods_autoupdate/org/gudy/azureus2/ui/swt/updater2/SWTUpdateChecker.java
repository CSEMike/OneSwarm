	    ...
	    if ( update_required ){
		        ...
		        if ( swt_url != null ){
		        	...
		        	if ( url_str.startsWith("jar:file:")){
		        		...
		        		String	expected_location;
		        		  ...
		        		  // *********************************************************
							/*
							 * ok, so they really need to get their paths straight...
							 */
							//if ( Constants.isOSX ){
							//    	  
	      ...
	      final Update update = 
	    	  checker.addUpdate("SWT Library for " + versionGetter.getPlatform(),
		          new String[] {"SWT is the graphical library used by OneSwarm Beta"},
	...
	InputStream 		data ) 
  {
    ...
    try {
      ...
      while((entry = zip.getNextEntry()) != null) {
        ...
        //*************************************************************
				/*
				 * the usual path things
				 */
				if (name.endsWith("swt.jar")) {
					...
					//          }
					installer.addMoveAction(name, SystemProperties.getSwtJar());
					...
					installer.addMoveAction(name, SystemProperties.getDllPath() + name);
