  ...
  StartServer()
    {
	  ...
	  System.out.println("starting custom start server...");
	  
    try {
    	// PIAMOD -- changed from 6880 -> 6885
        socket = new ServerSocket(6885, 50, InetAddress.getByName("127.0.0.1")); //NOLAR: only bind to localhost
        
        state = STATE_LISTENING;    
        
        if (Logger.isEnabled())
        	Logger.log(new LogEvent(LOGID, "StartServer: listening on "
        			+ "127.0.0.1:6885 for passed torrent info"));
      ...
      System.out.println( "StartServer ERROR: unable" + " to bind to 127.0.0.1:6885 listening"
