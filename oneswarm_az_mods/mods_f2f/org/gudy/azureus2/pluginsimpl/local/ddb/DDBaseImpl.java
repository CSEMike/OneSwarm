...
/*
 * EDITS in this class by: isdal
 * 
 * We need those 3 bytes :-)
 * 
 * Modified code so that if the value in not multi-value, allow 256 bytes
 *  if it is multi-value, allow 256-3 bytes to make room for the overflow
 *  marker and length encoding
 */
public class 
DDBaseImpl 
	implements DistributedDatabase
{
		...
		throws DistributedDatabaseException
	{
		...
		//****************************************************************
		if (values.length == 1) {
			if (((DDBaseValueImpl) values[0]).getBytes().length > DHT.MAX_VALUE_SIZE) {
				throw (new DistributedDatabaseException("Value size limited to "
						+ DHT.MAX_VALUE_SIZE + " bytes for single value data"));
			}
		} else if (values.length > 1) {
			for (int i = 0; i < values.length; i++) {
				if (((DDBaseValueImpl) values[i]).getBytes().length > DDBaseValueImpl.MAX_VALUE_SIZE) {
					throw (new DistributedDatabaseException("Value size limited to "
							+ DDBaseValueImpl.MAX_VALUE_SIZE + " bytes for multi-value data"));
				}
			}
		}
		//****************************************************************
