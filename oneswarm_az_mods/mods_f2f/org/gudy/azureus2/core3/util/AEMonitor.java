	...
	extends AEMonSem
{
	...
	//******************************************
	// Edit, by isdal
	// print debug messages we if try to get non approved 
	// AE locks while holding the big fat lock
	private final static Set<String> allowedLocks;
	static{
		allowedLocks = new HashSet<String>(2);
		allowedLocks.add("OutgoingMessageQueue:queue");
		allowedLocks.add("OutgoingMessageQueue:DN");
	}
	//******************************************
	public
	AEMonitor(
	...
	{
		//******************************************
		BigFatLock bigFatLock = BigFatLock.getInstance(true);
		if(bigFatLock != null){
			if(!allowedLocks.contains(name) && bigFatLock.isHeldByCurrentThread()){
				System.err.println("Tried to get AEMonitor ("+name+") while holding bigfatlock!!!");
				System.err.println("Locked by trace:\n" +bigFatLock.getLockedTrace());
				new Exception("Current location").printStackTrace();
			}
		}
		//******************************************
	...
	//******************************************
		BigFatLock bigFatLock = BigFatLock.getInstance(true);
		if(bigFatLock != null){
			if(bigFatLock.isHeldByCurrentThread()){
				new Exception("Tried to get AEMonitor, while holding bigfatlock!!!").printStackTrace();
			}
		}
		//******************************************
		if ( DEBUG ){
