...
TRTrackerUtils
{
	...
	//*******************************************
	// edits by isdal
	// we don't want to connect to .invalid addresses
	private static String[] BLACKLISTED_DOMAINS = {".invalid",".example.com",".test",".defined"};
	// ******************************************
		...
		throws IOException
	{
		...
		for (String blacklisted : BLACKLISTED_DOMAINS) {
			String b = blacklisted;
			if(!b.startsWith(".")){
				b="."+b;
			}
			if(domain.endsWith(b)){
				//System.out.println("ignoring domain '" + b + "'");
				throw new IOException("Ignoring trackers at: '*" + b + "'");
			}
		}
	}
