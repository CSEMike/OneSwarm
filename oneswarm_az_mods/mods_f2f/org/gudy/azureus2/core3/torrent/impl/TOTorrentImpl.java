	...
	implements TOTorrent
{
	 ...
	 * OneSwarm attributes
	 */
	public static final String OS_NO_SHARE				= "OneSwarmNoShare";
	public static final String OS_NO_TEXT_SEARCH		= "OneSwarmNoTextSearch";
	
	public static final String OS_ARTIST				= "os-artist";
	public static final String OS_ALBUM					= "os-album";
	
	public static final String OS_TAGS					= "os-tags";
	
	public static final String OS_SHA1					= FileListFile.KEY_SHA1_HASH;
	public static final String OS_ED2K					= FileListFile.KEY_ED2K_HASH;
	
	public static final String OS_NO_STREAM				= "os-dont_stream";
	
	public static final String OS_PERMISSIONS			= "os-permissions";
	
	/**
	 * These are modified to include the OneSwarm attributes we want to preserve during
	 * torrent transfer. 
	 */
	protected static final List	TK_ADDITIONAL_OK_ATTRS = 
		Arrays.asList(new String[]{ TK_COMMENT_UTF8, AZUREUS_PROPERTIES, TK_WEBSEED_BT, TK_WEBSEED_GR, 
				OS_NO_SHARE, OS_NO_TEXT_SEARCH, OS_ARTIST, OS_ALBUM, OS_TAGS, OS_SHA1, OS_ED2K});
	...
	public void setName(byte[] name, String areYouSure){
		// this might cause funky side effects, 
		// do not do this unless you know what is going on...
		if("IamSure".equals(areYouSure)){
			setName(name);
		}
	}
	...
	removeAdditionalProperties()
	{
		...
		while( it.hasNext()){
			...
			} else if( key.equals("attributes") ) { // special case to preserve tags
			
				Map new_attribs = new HashMap();
				
				if( additional_properties.get(key) != null &&
					additional_properties.get(key) instanceof Map ) {
					
					Object o = ((Map)additional_properties.get(key)).get(OS_TAGS);
					if( o != null ) {
						new_attribs.put(OS_TAGS, o);
						new_props.put(key, new_attribs);
					}
					// add sha1 hashed if we have locally created ones, or if we are reading this torrent from a remote host (no OS_HASHES_ADDED) 
					Object added = ((Map) additional_properties.get(key)).get(Sha1HashManager.OS_HASHES_ADDED);
					if (added == null || added instanceof byte[]
							&& Sha1HashManager.OS_HASHES_TYPE_LOCAL.equals(new String(
									(byte[]) added))) {
						if(added != null){
							System.err.println("adding locally created hashes");
						} else {
							System.err.println("adding hashes from remote torrent");
						}
						Object sha1 = ((Map) additional_properties.get(key)).get(OS_SHA1);
						if (sha1 != null) {
							System.err.println("added sha1");
							new_attribs.put(OS_SHA1, sha1);
							new_props.put(key, new_attribs);
						}
						Object ed2k = ((Map) additional_properties.get(key)).get(OS_ED2K);
						if (ed2k != null) {
							System.err.println("added ed2k");
							new_attribs.put(OS_ED2K, ed2k);
							new_props.put(key, new_attribs);
						}
						System.err.println("new_attribs=" + new_attribs.size());
					}
				}
			}
		}
