	...
	extends ProtocolDecoder
{
                    ...
                    public void 
	    				...
	    				String ignore )
	    			 {
	    				 ...
	    				 if (min.equals("XOR")) {
							...
							MIN_CRYPTO = CRYPTO_PLAIN | CRYPTO_XOR | CRYPTO_RC4
									| CRYPTO_AES | CRYPTO_SSL;
						} 
	    				 // ************************************************************
		...
		throws IOException
	{
		...
		if (_shared_secrets != null
				&& _shared_secrets[0] != null
				&& new String(_shared_secrets[0])
		...
		if (sslMatch == SslHandShakeMatch.NO_SSL
				|| sslMatch == SslHandShakeMatch.NOT_ENOUGH_BYTES || sslMatch == null) {
			...
			while( loop ){
				...
				if (selected_protocol == CRYPTO_SSL) {
					if (sslFilter.isHandshakeCompleted()) {
						...
						// hosts we intended
						if (outBoundSSL) {
							byte[] remoteKey = (byte[]) transport.getUserData(
									OneSwarmSslTransportHelperFilterStream.REMOTE_SSL_PUBLIC_KEY);
							if(Arrays.equals(expectedPublicKey, OneSwarmSslTransportHelperFilterStream.ANY_KEY_ACCEPTED_BYTES)){
					...
					write_buffer = ByteBuffer
							.allocate(OneSwarmSslTransportHelperFilterStream.SSL_APP_BUFFER_SIZE);
					sslFilter = new OneSwarmSslTransportHelperFilterStream(transport,
							outbound, OneSwarmSslTransportHelperFilterStream.SslHandShakeMatch.SSL_CLIENT_CERT);
