...
IncomingConnectionManager 
{
	...
	{ 
        ...
        // EDIT by isdal:
        // Check mappers with a port specified first
	    outer:
        for (Boolean strictPort : new Boolean[] { Boolean.TRUE, Boolean.FALSE }) {
            for (Iterator i = match_buffers_cow.entrySet().iterator(); i.hasNext();) {
                ...
                // Loop through twice, first require a specific port
                if (strictPort && specific_port == -1){
                    continue;
                }
                ...
                if (min_match) {
                    if (orig_position < bm.minSize()) { // not enough bytes yet
                                                        // to compare
                        continue;
                    }
                    ...
                    if (orig_position < bm.matchThisSizeOrBigger()) { 
                        continue;
                    }
