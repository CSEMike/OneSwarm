...
public class ConfigurationDefaults {
        ...
        try {
            ...
            if (configdefaults == null) {
                ...
                try {
                    ...
                    // this is here for when we are just using a few of the
                    // Azureus classes and
                    // we can live with no defaults (e.g. swing webui). If we
                    // initialise
                    // the normal config-defaults fully this pulls in all sorts
                    // of unwanted
                    // classes (platform manager for example). Also, don't using
                    // Debug/Logging
                    // to record this fact!

                    System.out
                            .println("Falling back to default defaults as environment is restricted");
    ...
    protected ConfigurationDefaults() {
        ...
        def.put("oneswarm.max.ui.tags", new Long(300));
        def.put("oneswarm.max.ui.search.results", new Long(500));
        def.put("oneswarm.max.sha1.hash.rate.kbps", new Long(10 * 1024));
        ...
        def.put("Override Ip", "");
        def.put("Enable incremental file creation", TRUE); // PIAMOD -- great
                                                           // for encrypted
                                                           // partitions. speeds
                                                           // up everything
                                                           // generally
        ...
        def.put("UDP.NonData.Listen.Port", new Long(6881)); // two effective
                                                            // enablers for
                                                            // this, dht +
                                                            // tracker udp
                                                            // client
        def.put("UDP.NonData.Listen.Port.Same", TRUE); // control over whether
                                                       // non-data and data udp
                                                       // port are the same
        ...
        def.put("Max.Peer.Connections.Per.Torrent", new Long(
                COConfigurationManager.CONFIG_DEFAULT_MAX_CONNECTIONS_PER_TORRENT));
        def.put("Max.Peer.Connections.Per.Torrent.When.Seeding", new Long(
                COConfigurationManager.CONFIG_DEFAULT_MAX_CONNECTIONS_PER_TORRENT));
        def.put("Max.Peer.Connections.Per.Torrent.When.Seeding.Enable", FALSE);
        def.put("Max.Peer.Connections.Total", new Long(
        ...
        def.put("AutoSpeed Available", FALSE); // informative read-only
                                               // parameter
        ...
        def.put("Check Pieces on Completion", FALSE); // PIAMOD
        def.put("Stop Ratio", new Float(0));
        def.put("Stop Peers Ratio", ZERO);
        def.put("Disconnect Seed", TRUE);
        def.put("Seeding Piece Check Recheck Enable", FALSE); // PIAMOD
        ...
        long DEFAULT_UPDATE_CHECK = System.getProperty("oneswarm.integration.test") == null ? TRUE
                : FALSE;
        ...
        def.put("Proxy.Host", "");
        def.put("Proxy.Port", "");
        def.put("Proxy.Username", "<none>"); // default is explicit "none", as
                                             // opposed to "not defined"
        ...
        def.put("Prioritize First Piece", TRUE); // PIAMOD -- this needs to be
                                                 // true for streaming DL
        ...
        def.put("Tracker Client Send OS and Java Version", FALSE); // PIAMOD --
                                                                   // um, no!
        ...
        def.put("Tracker Announce Cache Min Peers", new Long(
                TRTrackerServer.DEFAULT_ANNOUNCE_CACHE_PEER_THRESHOLD));
        ...
        def.put("Tracker Scrape Retry Percentage", new Long(
                TRTrackerServer.DEFAULT_SCRAPE_RETRY_PERCENTAGE));
        ...
        // *********************************************
        // EDIT by: isdal
        // not scraping stopped torrents
        // def.put( "Tracker Client Scrape Stopped Enable", TRUE);
        def.put("Tracker Client Scrape Stopped Enable", FALSE);
        // *********************************************
        ...
        // *******************************************************
        def.put("Network Selection Default.OSFriendNetwork", TRUE);
        def.put("Network Selection Default.WebSeed", TRUE);
        // *******************************************************
        ...
        def.put("diskmanager.perf.cache.notsmallerthan", new Long(1024)); // 1024
                                                                          // K
        ...
        if (Constants.isOSX) {
            def.put("Enable System Tray", FALSE); // PIAMOD -- was true? default
                                                  // to off for OS X, all other
        ...
        def.put("add_torrents_silently", TRUE); // PIAMOD -- so we don't keep
                                                // reopening the web UI over and
                                                // over
        ...
        // This gets permanently set by GlobalManagerImpl to false once Azureus
        // has initialised.
        ...
        def.put(SpeedManagerAlgorithmProviderV2.SETTING_DOWNLOAD_MAX_LIMIT, new Long(
                SMConst.START_DOWNLOAD_RATE_MAX));
        def.put(SpeedManagerAlgorithmProviderV2.SETTING_UPLOAD_MAX_LIMIT, new Long(
        ...
        def.put(SpeedManagerImpl.CONFIG_VERSION, new Long(2)); // 1 == classic,
                                                               // 2 == beta

        def.put(SpeedLimitMonitor.DOWNLOAD_CONF_LIMIT_SETTING,
        ...
        // default V2 algorithm seeding and download mode useage, stored as an
        // Int
        ...
        /*
         * Edit, by isdal
         * added default oneswarm privacy parameters
         */
    ...
    protected void def_put(String key, String key_def,
    ...
    private void checkParameterExists(String p) throws ConfigurationParameterNotFoundException {
        if (!def.containsKey(p)) {
            ConfigurationParameterNotFoundException cpnfe = new ConfigurationParameterNotFoundException(
                    p);
    ...
    protected void runVerifiers() {
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < verifiers.size(); i++) {
                ...
                COConfigurationManager.ParameterVerifier verifier = (COConfigurationManager.ParameterVerifier) verifiers
                        .get(i);
                ...
                if (!verifier.verify(key, val)) {
                    ...
                    Debug.out("Parameter '" + key + "', value '" + val
                            + "' failed verification - setting back to default '" + val_def + "'");
