...
import edu.washington.cs.oneswarm.f2f.multisource.Sha1Peer;
	...
	implements PiecePicker
{
	 ...
	 * constant deciding how much to add so pieces are downloaded in order 
	 * 
	 * pieces gets a boost that decreases linearly with piece number
	 * 
	 * PRIORITY_IN_ORDER_FILES - ("piece number in file" * ("total piece number"/PRIORITY_IN_ORDER_FILES))
	 * 
	 * should tip the balance a bit 
	 */
	public static final int							 PRIORITY_IN_ORDER_FILES				= 5000;
	...
	//********************************************************
	// Edit, for f2f we need this to be a bit more aggressive
	/** Number of bytes to get from the end of the file **/
	public static final int							 REQUESTS_LAST_BYTES						= 1024 * 1024;

	/** Min number of requests sent to a peer */
	//private static final int REQUESTS_MIN	=4;
	...
	private static final long						 END_GAME_MODE_TIMEOUT					= 60
	...
	private static final int							NO_REQUEST_BACKOFF_MAX_LOOPS	 = NO_REQUEST_BACKOFF_MAX_MILLIS
																																					 / PeerControlScheduler.SCHEDULE_PERIOD_MILLIS;
	...
	private final AEMonitor							 endGameModeChunks_mon					= new AEMonitor(
																																					 "PiecePicker:EGM");
	...
	// for a certain user/torrent combo
	private long piecePickerSha1GroupingRandomSeed;	
	...
	static {
		...
		COConfigurationManager.addParameterListener(
				"Prioritize Most Completed Files", parameterListener);
		COConfigurationManager.addAndFireParameterListener(
				"Prioritize First Piece", parameterListener);
		COConfigurationManager.addAndFireParameterListener(
				"Piece Picker Request Hint Enabled", parameterListener);
	...
	public PiecePickerImpl(final PEPeerControl pc) {
		...
		System.out.println("* Using in-order aware piece picker");
		...
		try{
		this.piecePickerSha1GroupingRandomSeed = id.hashCode()
				^ Arrays.hashCode(diskManager.getTorrent().getHash());
		}catch(Exception e){
			e.printStackTrace();
		}
	}
    ...
    public final void updateAvailability() {
		...
		if (now >= time_last_avail && now < time_last_avail + TIME_MIN_AVAILABILITY)
			return;
		if (availabilityDrift > 0 || now < time_last_rebuild
				|| (now - time_last_rebuild) > timeAvailRebuild) {
			try {
				availabilityMon.enter();
				...
				if (Constants.isCVSVersion()) {
					final int[] old_availability = availabilityAsynch == null
					...
					if (errors > 0 && errors != nbPieces) {
						if (Logger.isEnabled())
							Logger.log(new LogEvent(peerControl, LOGID, LogEvent.LT_ERROR,
									"updateAvailability(): availability rebuild errors ="
		...
		for (i = 0; i < nbPieces; i++) {
			...
			if (avail > 0 && avail < rarestMin && dmPiece.isDownloadable()
					&& (pePiece == null || pePiece.isRequestable()))
		...
		for (i = 0; i < nbPieces; i++) {
			...
			if (avail > 0) {
				if (avail > allMin)
					total++;
				if (avail <= rarestMin && dmPiece.isDownloadable() && pePiece != null
						&& !pePiece.isRequested())
					rarestActive++;
				totalAvail += avail;
			}
	...
	private final int[] recomputeAvailability() {
		if (availabilityDrift > 0 && availabilityDrift != nbPieces
				&& Logger.isEnabled())
			Logger.log(new LogEvent(diskManager.getTorrent(), LOGID,
					LogEvent.LT_INFORMATION, "Recomputing availabiliy. Drift="
							+ availabilityDrift + ":" + peerControl.getDisplayName()));
    ...
    public final void allocateRequests() {
		...
		if (priorityRTAexists) {
			...
			long best_block_eta = SystemTime.getCurrentTime()
					+ getNextBlockETAFromNow(best_uploader);
			...
			try {
				while (allocated_request && priorityRTAexists) {
					...
					while (!block_time_order_peers.isEmpty()) {
						...
						int maxRequests = REQUESTS_MIN
								+ (int) (pt.getStats().getDataReceiveRate() / SLOPE_REQUESTS)
								+ 1;
		...
		for (int i = 0; i < uploadersSize; i++) {
			...
			if (dispenser.peek(DiskManager.BLOCK_SIZE) < 1
					&& (!pt.isLANLocal() || includeLanPeersInReqLimiting))
				break;
			...
			if (pt.isDownloadPossible()) {
				...
				//***************************************************
				/* Edit, by isdal
				 * For f2f we need to be more aggressive here since the bdp is much higher
				 * Also, peers only get scheduled every second or so forcing us to keep 
				 * a large window to get any speed on multihop links with high bdp
				 */
				//if (pt.getNbRequests() <= (maxRequests * 3) / 5) {
				if (pt.getNbRequests() <= maxRequests) {
					//System.err.println(System.currentTimeMillis() + "\trequesting from peer; speed:"+pt.getStats().getDataReceiveRate()+" outstanding requests:"+pt.getNbRequests()+" max:"+maxRequests);
					...
					try {
						...
						while (pt.isDownloadPossible() && pt.getNbRequests() < maxRequests) {
							...
							if (peer_managing_requests || !endGameMode) {
								//***************************************************
								// EDIT: lets request 10% of the block at the time to 
								// decrease the burstiness 
								//allocated = findPieceToDownload(pt, maxRequests );
								allocated = findPieceToDownload(pt, Math.max(1,
										Math.round(0.1f * maxRequests)));
								//allocated = findPieceToDownload(pt, 1);
								//***************************************************
							} else {
					...
					//System.err.println(System.currentTimeMillis() + "\tdone requesting from peer; allocated:"+total_allocated+" outstanding requests:"+pt.getNbRequests()+" max:"+maxRequests);
		...
		for (int i = 0; i < rarestStartedPieces.size(); i++) {
			...
			if ((rarestStarted.getAvailability() <= globalMinOthers || globalMinOthers > globalMin)
					&& (SystemTime.getCurrentTime()
							- rarestStarted.getLastDownloadTime(SystemTime.getCurrentTime()) < 60 * 1000 || rarestStarted.getNbWritten() == 0)
	...
	private final void computeBasePriorities() {
		...
		if (!priorityRTAexists) {
			if (startPriorities != null
					&& ((now > timeLastPriorities && now < time_last_avail
							+ TIME_MIN_PRIORITIES) || (priorityParamChange >= paramPriorityChange
		...
		try {
			...
			final int nbConnects = peerControl.getNbPeers()
					+ peerControl.getNbSeeds();
			...
			for (int i = 0; i < nbPieces; i++) {
				...
				//****************************************************
				// edit: by isdal
				// for auto added downloads
				// do not request pieces that already are downloaded by
				// other sha1 peers
				DownloadManager dm = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManager(
						new HashWrapper(peerControl.getHash()));

				if (dm != null
						&& dm.getDownloadState().getBooleanAttribute(
								Sha1DownloadManager.ONESWARM_AUTO_ADDED)) {
					List peers = peerControl.getPeers();
					for (Object o : peers) {
						if (o instanceof Sha1Peer) {
							Sha1Peer p = (Sha1Peer) o;
							if (p.isPieceAvailable(i)) {
								dmPiece.setNeeded(false);
								continue piece_loop;
							}
						}
					}
				...
				//*****************************************************
				// Edit: by isdal
				// fixed straming, no streaming on files
				boolean streaming = false;
				if (dm != null) {
					// only stream if the stream atttribute is set
					if (dm.getDownloadState().getBooleanAttribute(
				...
				for (int j = 0; j < pieceListSize; j++) {
					...
					if (length > 0 && downloaded < length && !fileInfo.isSkipped()) {
						...
						if (firstPiecePriorityL
								&& fileInfo.getNbPieces() > FIRST_PIECE_MIN_NB) {
						...
						/*
						 * EDIT: made by isdal@cs.washington.edu
						   * prioritise first blocks of media files, see top for details
						   * the first file gets the highest boost and so on...
						   * 
						   * EDIT: made by piatek (unless this has been overridden by the user -- this 
						   * can make things faster in certain circumstances)
						  */

						if (streaming) {
							//int fileIndex = fileInfo.getIndex();
							// int firstPieceInFile = fileInfo.getFirstPieceNumber();
							// double currentRelativePieceNumber = i - firstPieceInFile;
							double totalPieceNum = diskManager.getNbPieces();

							int boost = (int) Math.round(PRIORITY_IN_ORDER_FILES
									- (i * (PRIORITY_IN_ORDER_FILES / totalPieceNum)));
							priority += boost + PRIORITY_OVERRIDES_RAREST;

							// high prio for first/last piece

							int firstLastNum = REQUESTS_LAST_BYTES
									/ diskManager.getPieceLength();
							if (i >= fileInfo.getLastPieceNumber() - firstLastNum
									&& i <= fileInfo.getLastPieceNumber()) {
								priority += PRIORITY_IN_ORDER_FILES;
							}
							if (i <= fileInfo.getFirstPieceNumber() + firstLastNum
									&& i >= fileInfo.getFirstPieceNumber()) {
								priority += PRIORITY_IN_ORDER_FILES;
							}

							// System.out.println("boosting priority of piece: " +i +
							// "(" + currentRelativePieceNumber +") with " + boost);
						} else {
							// have a slight prio to chunk things into larger blocks 
							// to improve potential sha1 multisource matching 
							final int totalRanges = Math.min(diskManager.getNbPieces() / 10, 50);
							final int rangePrioMultiplier = PRIORITY_W_COMPLETION/totalRanges;

							// create priority ranges to improve sha1 multi source
							ArrayList<Integer> rangePrios = new ArrayList<Integer>(totalRanges);
							for (int k = 0; k < totalRanges; k++) {
								rangePrios.add(k * rangePrioMultiplier);
							}
							Collections.shuffle(rangePrios, new Random(piecePickerSha1GroupingRandomSeed));

							double pieceCount = diskManager.getNbPieces();
							int piecesPerRange = (int) Math.ceil(pieceCount / totalRanges);

							int range = i / piecesPerRange;
							double pieceInRange = i % piecesPerRange;
							double piecePrio = pieceInRange / piecesPerRange;
							priority += Math.round(rangePrios.get(range) + piecePrio);
						}
						//******************************************************************
						...
						if (fileInfo.isPriority()) {
							...
							if (completionPriorityL) {
								...
								if (percent >= 900)
									priority += (PRIORITY_W_COMPLETION * downloaded)
											/ diskManager.getTotalLength();
							}
						}
	...
	protected final int findPieceToDownload(PEPeerTransport pt, int nbWanted) {
		...
		else { //create piece manually
			int[] peer_priority_offsets = pt.getPriorityOffsets();

			int this_offset = peer_priority_offsets == null ? 0
					: peer_priority_offsets[pieceNumber];

			//create piece manually

			pePiece = new PEPieceImpl(pt.getManager(), dmPieces[pieceNumber],
					peerSpeed >> 1);
		...
		if (!pt.isLANLocal() || includeLanPeersInReqLimiting
				&& !pt.getPeerSource().startsWith("SHA1"))
		...
		for (int i = 0; i < nbBlocks; i++) {
			...
			if (pt.request(pieceNumber, thisBlock * DiskManager.BLOCK_SIZE,
					pePiece.getBlockSize(thisBlock)) != null) {
				requested++;
		...
		if (requested > 0 && pePiece.getAvailability() <= globalMinOthers
				&& calcRarestAllowed() > 0 && !rarestStartedPieces.contains(pePiece))
	...
	protected final boolean findRTAPieceToDownload(PEPeerTransport pt,
			boolean best_uploader, long best_uploader_next_block_eta) {
		...
		try {
			...
			for (int i = startI; i <= endI; i++) {
				...
				if (peerHavePieces.flags[i] && startPriorities[i] == PRIORITY_REALTIME
						&& piece_rta > 0) {
					...
					boolean try_allocate_even_though_late = my_next_block_eta > piece_rta
							&& best_uploader_next_block_eta > piece_rta;

					if (piece_rta >= piece_min_rta_time) {

						// piece is less urgent than an already found one

					} else if (my_next_block_eta > piece_rta
							&& !(best_uploader || best_uploader_next_block_eta > piece_rta)) {
						...
						// get in time and leave the stuck ones for just the best uploader to get
							...
							|| (realtime_data = pePiece.getRealTimeData()) == null) {
						...
						for (int j = 0; j < peer_requests.length; j++) {
							...
							while (it.hasNext()) {
								...
								if (request_index == -1) {
									...
									if (LOG_RTA)
										rta_log_str += "{request_lost="
												+ this_request.getPieceNumber() + "}";
								...
								int next_block_bytes = (request_index + 1)
										* DiskManager.BLOCK_SIZE;

								long this_peer_eta = now
										+ ((next_block_bytes * 1000) / this_up_bps);
							...
							if (!pt_already_present) {
								...
								if (block_peer_requests.size() == 0) {
									...
									if (LOG_RTA)
										rta_log_str += "{alloc as no req=" + i + ",block=" + j
												+ ",time=" + (piece_rta - now) + "}";
									...
									if (my_next_block_eta < best_eta) {
										...
										if (LOG_RTA)
											rta_log_str += "{taking over, time="
													+ (best_eta - my_next_block_eta) + "}";
			...
			if (piece_min_rta_index != -1) {
				...
				if (LOG_RTA)
					rta_log_str += ",{select_piece=" + piece_min_rta_index + ",block="
							+ piece_min_rta_block + ",time=" + (piece_min_rta_time - now)
							+ "}";
				...
				if (dispenser.dispense(1, DiskManager.BLOCK_SIZE) == 1
					...
					if (pePiece == null) {
						...
						pePiece = new PEPieceImpl(pt.getManager(),
								dmPieces[piece_min_rta_index], peerSpeed >> 1);
					...
					DiskManagerReadRequest request = pt.request(piece_min_rta_index,
							piece_min_rta_block * DiskManager.BLOCK_SIZE,
							pePiece.getBlockSize(piece_min_rta_block));
	...
	private final int getRequestCandidate(final PEPeerTransport pt) {
		...
		if (reservedPieceNumber >= 0) {
			...
			if (pePiece != null) {
				...
				if (peerReserved != null && peerReserved.equals(pt.getIp())) {
					...
					if (peerHavePieces.flags[reservedPieceNumber]
							&& pePiece.isRequestable()) {
			...
			if (peerHavePieces.flags[i]) {
				...
				if (priority >= 0 && dmPiece.isDownloadable()) {
					...
					if (pePiece == null || pePiece.isRequestable()) {
						...
						boolean pieceRarestOverride = priority >= PRIORITY_OVERRIDES_RAREST
								? true : rarestAllowed;
						...
						if (pePiece != null) {
							...
							boolean rarestPrio = avail <= globalMinOthers
									&& (startedRarest || rarestAllowed);
							...
							if (pt.isSnubbed()) {
								...
								mayResume &= peerSpeed < 2 || pieceSpeed > 0
							...
							if (secondChoiceResume == -1
									|| avail > availability[secondChoiceResume])
							...
							priority += pePiece.getTimeSinceLastActivity()
									/ PRIORITY_DW_STALE;
							// how long since piece was started
							pieceAge = now - pePiece.getCreationTime();
							if (pieceAge > 0)
								priority += PRIORITY_W_AGE * pieceAge
										/ (PRIORITY_DW_AGE * dmPiece.getNbBlocks());
							// how much is already written to disk
							priority += (PRIORITY_W_PIECE_DONE * dmPiece.getNbWritten())
									/ dmPiece.getNbBlocks();

							pePiece.setResumePriority(priority); // this is only for display

							if (avail < resumeMinAvail
									|| (avail == resumeMinAvail && priority > resumeMaxPriority)) { // this piece seems like best choice for resuming
		...
		if (!forceStart || startCandidates == null || startCandidates.nbSet <= 0) {
			...
			if (reservedPieceNumber >= 0
					&& (resumeIsRarest || !startIsRarest || !rarestAllowed
							|| startCandidates == null || startCandidates.nbSet <= 0))
				return reservedPieceNumber;
			...
			if (reservedPieceNumber >= 0 && globalMinOthers > 0
					&& peerControl.getNbActivePieces() > 32) // check at arbitrary figure of 32 pieces 
			{
				...
				if (Constants.isCVSVersion() && Logger.isEnabled())
					Logger.log(new LogEvent(new Object[] {
						pt,
						peerControl
					}, LOGID, "Start/resume choice; piece #:" + reservedPieceNumber
							+ " resumeIsBetter:" + resumeIsBetter + " globalMinOthers="
							+ globalMinOthers + " startMaxPriority=" + startMaxPriority
							+ " startMinAvail=" + startMinAvail + " resumeMaxPriority="
							+ resumeMaxPriority + " resumeMinAvail=" + resumeMinAvail + " : "
							+ pt));
	...
	private final void checkEndGameMode() {
		...
		long trigger = use_rta_egm ? RTA_END_GAME_MODE_SIZE_TRIGGER
				: END_GAME_MODE_SIZE_TRIGGER;
		...
		if (remaining <= trigger) {
			...
			try {
				...
				if (Logger.isEnabled())
					Logger.log(new LogEvent(diskManager.getTorrent(), LOGID,
							"Entering end-game mode: " + peerControl.getDisplayName()));
	...
	protected final int findPieceInEndGameMode(final PEPeerTransport pt,
			final int wants) {
		...
		try {
			...
			if (endGameMode) {
				...
				if (Logger.isEnabled()) {
					...
					Logger.log(new LogEvent(diskManager.getTorrent(), LOGID,
							"Leaving end-game mode: " + peerControl.getDisplayName()));
				}
	...
	protected void abandonEndGameMode() {
		if (!endGameModeAbandoned) {
			...
			try {
				...
				if (Logger.isEnabled())
					Logger.log(new LogEvent(diskManager.getTorrent(), LOGID,
							"Abandoning end-game mode: " + peerControl.getDisplayName()));
    ...
    public String getPieceString(int piece_number) {
		...
		if (priority == PRIORITY_REALTIME) {
			...
			str = "pri=rta:"
					+ (rta == null ? "?"
							: ("" + (rta[piece_number] - SystemTime.getCurrentTime())));
