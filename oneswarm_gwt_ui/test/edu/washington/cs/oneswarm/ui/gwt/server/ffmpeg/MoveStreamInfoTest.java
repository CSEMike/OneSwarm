				...
				for (int i = 0; i < files.length; i++) {
					...
					if (file.isFile()) {
						...
						Process ffmpeg = Runtime.getRuntime().exec(
								new String[] { "/opt/local/bin/ffmpeg", "-i",
										file.getCanonicalPath(), "-vcodec",
										"png", "-ss", seekTo + "", "-vframes",
										"1", "-an", "-f", "rawvideo", "-" });

						BufferedReader stdErr = new BufferedReader(
								new InputStreamReader(ffmpeg.getErrorStream()));
						// dump anything showing up on stdout
						StreamReader r = new StreamReader(ffmpeg
								.getInputStream());
						...
						if (exitVal == 0) {
							MovieStreamInfo m = new MovieStreamInfo(output
									.toString());
