...
public class StatsHandler extends ServletHandler {
    ...
    enum TimeSeriesData {
        ...
        List<Number> timeSeriesData = (List<Number>) Collections
                .synchronizedList(new LinkedList<Number>());
    ...
    public StatsHandler(CoreInterface coreInterface) {
        ...
        TimerTask ul_dl = new TimerTask() {
            public void run() {
                TimeSeriesData.UPLOAD.update(core.getGlobalManager().getStats()
                        .getDataAndProtocolSendRate() / 1024);
                TimeSeriesData.DOWNLOAD.update(core.getGlobalManager().getStats()
                        .getDataAndProtocolReceiveRate() / 1024);
            }
        };
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        url.append("http://chart.apis.google.com/chart?" + "chs=325x300" + "&chd=t:");
        ...
        url.append("&cht=lxy" + "&chxt=x,y" + "&chds=0," + (sampled.size() * granularity) / xtic
                + ",0," + maxVal + "&chxr=0,0," + (sampled.size() * granularity) / xtic + "|1,0,"
                + maxVal);
