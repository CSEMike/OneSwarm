...
public class CoordinatorExecutor extends Thread {
    ...
    public CoordinatorExecutor(CoordinatorHeartbeatThread parent, byte[] commands)
            throws IOException {
        this.parent = parent;
            ...
            for (String s : commands) {
                ...
                if (toks[0].equals("ok")) {
                    ...
                    if (UIFunctionsManager.getUIFunctions().requestShutdown() == false) {
                        System.err
                                .println("Requested shutdown failed, so we are halting forcefully.");
                    ...
                    if (toDel != null) {
                        coreInterface.getF2FInterface().deleteFriend(
                                FriendInfoLiteFactory.createFriendInfo(toDel));
                    } else {
                        logger.warning("couldn't find friend to delete for key: " + toks[1]);
                    }
                } else if (toks[0].equals("blockgroup")) {
                    for (FriendInfoLite flite : coreInterface.getF2FInterface().getFriends(true,
                            true)) {
                    ...
                    for (FriendInfoLite flite : coreInterface.getF2FInterface().getFriends(true,
                        ...
                        if (f.getGroup().equals(toks[1])) {
                            coreInterface.getF2FInterface().deleteFriend(
                                    FriendInfoLiteFactory.createFriendInfo(f));
                        }
                    }
                    ...
                    FriendInfoLite[] allFriends = coreInterface.getF2FInterface().getFriends(true,
                            false);
                    FriendInfoLite[] onlineFriends = coreInterface.getF2FInterface().getFriends(
                            false, false);
                    ...
                    logger.info("Total friends: " + allFriends.length + " onlineToBlock: "
                            + onlineToBlock + " online: " + onlineFriends.length);
                    for (FriendInfoLite flite : allFriends) {
                        ...
                        if (f.isConnected() == false) {
                            ...
                            if (onlineBlocked < onlineToBlock) {
                                ...
                                logger.info("Skipping block of online user: " + f.getNick()
                    ...
                    Runtime.getRuntime().exec(
                            new String[] { "/usr/bin/at", "-f", "/tmp/atcmds", "now" });
    ...
    private void downloadTorrentAndStart(final String url, final long at,
    ...
    private void createRandomAndShare(String name, long sizeBytes) throws IOException,
            TOTorrentException {
    ...
    private void downloadAndShare(String swarmName, String url) throws MalformedURLException,
    ...
    private void downloadAndStart(final String base64hash, final long when) throws IOException,
            TOTorrentException {
    ...
    private DownloadManager addAndStartTorrent(TOTorrent torrent) throws TOTorrentException,
            IOException {
        return addAndStartTorrent(torrent, null);
    }

    private DownloadManager addAndStartTorrent(TOTorrent torrent, File outFile)
    ...
    private void addFriend(String group, String keybase64, boolean allowChat, boolean shareFileList)
