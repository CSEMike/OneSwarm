...
public class CreateSwarmDialogBox extends OneSwarmDialogBox {
	...
	protected CreateSwarmDialogBox onInitialized() {
		permissionsClickListener = new ClickHandler() {
			public void onClick(ClickEvent event) {
				OneSwarmDialogBox dlg = new AprioriPermissionsDialog(msg.create_swarm(), msg.create_swarm_visiblity_msg(), permitted_groups, new ApriorPermissionsCallback() {
	...
	private VerticalPanel createSharePanel() {
		...
		simple_perms_hp.setCellHorizontalAlignment(sharePubliclyCheckBox, HorizontalPanel.ALIGN_LEFT);
		...
		defaultTrackersListBox.addChangeHandler(new ChangeHandler() {
			public void onChange(ChangeEvent event) {
				if (defaultTrackersListBox.getItemText(defaultTrackersListBox.getSelectedIndex()).equals(CUSTOM_TRACKER_ITEM)) {
					String tracker = Window.prompt(msg.create_swarm_trackers_msg(), "http://tracker.oneswarm.net:6969/announce");
					if (tracker != null) {
						...
						defaultTrackersListBox.setItemSelected(defaultTrackersListBox.getItemCount() - 1, true);
	...
	private void reflect_advanced_permissions(ArrayList<PermissionsGroup> inGroups) {
		sharePubliclyCheckBox.setValue(false);
		boolean sharePublic = inGroups.contains(new PermissionsGroup(PermissionsGroup.PUBLIC_INTERNET));
		if (sharePublic) {
	...
	private void setPath(String inPath) {
		...
		if (inPath == null) {
			 ...
			 * overflow...
			 */
			String truncated = inPath.substring(Math.max(0, inPath.length() - MAX_PATH_DISPLAYED), inPath.length());
	...
	public void onClick(ClickEvent event) {
		if (event.getSource().equals(cancelButton)) {
			...
			OneSwarmRPCClient.getService().selectFileOrDirectory(OneSwarmRPCClient.getSessionID(), useDirChooser, new AsyncCallback<String>() {
				...
				public void onSuccess(String path) {
					...
					if (multDirRadioButton.getValue() && path != null) {
						...
						System.out.println(multiDirPanel.getOffsetWidth() + " multidirpanel width");
					}
				}
			});
		} else {
	...
	private void createSwarm(final ArrayList<String> paths) {
		if (paths.size() > 0) {
			...
			OneSwarmRPCClient.getService().createSwarmFromLocalFileSystemPath(OneSwarmRPCClient.getSessionID(), mPath, paths, true, defaultTrackersListBox.getItemText(defaultTrackersListBox.getSelectedIndex()), permitted_groups, new AsyncCallback<Boolean>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}
				...
				public void onSuccess(Boolean result) {
					...
					if (result == false) {
						...
						/**
						 * This will be picked up by the new background task
						 * manager
						 */
