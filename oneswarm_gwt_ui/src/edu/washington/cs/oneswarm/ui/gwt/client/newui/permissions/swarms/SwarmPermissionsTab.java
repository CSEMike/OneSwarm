...
//		fileTable.setSortingEnabled(false);
		...
		OneSwarmRPCClient.getService().getAllGroups(OneSwarmRPCClient.getSessionID(), new AsyncCallback<ArrayList<PermissionsGroup>>(){
			...
			public void onSuccess(ArrayList<PermissionsGroup> result) {
				...
				if( all_groups != null )
				{
					...
					OneSwarmRPCClient.getService().getGroupsForSwarm(OneSwarmRPCClient.getSessionID(), inSwarm, new AsyncCallback<ArrayList<PermissionsGroup>>(){
						public void onFailure(Throwable caught) {
							caught.printStackTrace();
						}
	...
	private void changeSelectedSwarmUI( final TorrentInfo inSwarm, final List<PermissionsGroup> all_groups, List<PermissionsGroup> swarm_groups )
	{
		...
		final MembershipList<PermissionsGroup> available_groups = new MembershipList<PermissionsGroup>(msg.visibility_group_available_groups(), true, all_groups, swarm_groups, true, new GroupsListSorter());
		final MembershipList<PermissionsGroup> sharing_with_groups = new MembershipList<PermissionsGroup>(msg.visibility_sharing_with(), false, all_groups, all_sub_shared, true, new GroupsListSorter());
		...
		HorizontalPanel hp = new HorizontalPanel() {
			public void onDetach() {
				System.out.println("attempting to synchronize permissions for: " + inSwarm.getName());
				
				ArrayList<PermissionsGroup> groupsList = sharing_with_groups.getMembers();
				
				OneSwarmRPCClient.getService().setGroupsForSwarm(OneSwarmRPCClient.getSessionID(), inSwarm, groupsList, new AsyncCallback<ReportableException>(){
