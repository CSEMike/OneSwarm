...
public class InvitationViewPanel extends FriendsImportWithBack {
	...
	private class InvitationViewStep extends FriendsImportWithBackStep {
		...
		public void refresh() {
			...
			OneSwarmRPCClient.getService().getSentFriendInvitations(OneSwarmRPCClient.getSessionID(), new AsyncCallback<ArrayList<FriendInvitationLite>>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}

				public void onSuccess(ArrayList<FriendInvitationLite> result) {
					FriendInvitationLite[] invitations = result.toArray(new FriendInvitationLite[result.size()]);
					if (invitations.length > 0) {
						...
						sentTablePanel.setCellHorizontalAlignment(l, VerticalPanel.ALIGN_CENTER);
					}
				}
			});

			redeemedTablePanel.clear();
			OneSwarmRPCClient.getService().getRedeemedFriendInvitations(OneSwarmRPCClient.getSessionID(), new AsyncCallback<ArrayList<FriendInvitationLite>>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}
				...
				public void onSuccess(ArrayList<FriendInvitationLite> result) {
					...
					if (invitations.length > 0) {
						...
						redeemedTablePanel.setCellHorizontalAlignment(l, VerticalPanel.ALIGN_CENTER);
					}
	...
	private static class InvitationTable extends ScrollTable {
		...
		enum Column {
			CHECKBOX("", 0, 10), NICKNAME(msg.add_friends_invite_view_table_nickname(), 1, 45), CREATED(msg.add_friends_invite_view_table_created(), 2, 45), STATUS(msg.add_friends_invite_view_table_status(), 3, 45), EXPIRES(msg.add_friends_invite_view_table_expires(), 4, 45);
		...
		public void removeClicked() {
			...
			if (Window.confirm(msg.add_friends_invite_view_confirm_delete(selectedInvitations.size()))) {

				OneSwarmRPCClient.getService().deleteFriendInvitations(OneSwarmRPCClient.getSessionID(), selectedInvitations, new AsyncCallback<Void>() {
					public void onFailure(Throwable caught) {
		...
		public InvitationTable(FriendInvitationLite[] invitations, boolean createdLocally) {
			super(new FixedWidthGrid(0, createdLocally ? Column.values().length : Column.values().length - 1), new FixedWidthFlexTable());
			mData = getDataTable();
			...
			TableListener listener = new TableListener() {
				public void onCellClicked(SourcesTableEvents check, int row, int column) {
					if (column >= 1) {
						...
						FriendInvitationLite invitation = ((FriendInvitationCheckBox) mData.getWidget(row, 0)).f;
						...
						dlg.center();
						dlg.setPopupPosition(dlg.getAbsoluteLeft() + 40, InvitationTable.this.getAbsoluteTop());
						dlg.setVisible(true);
			...
			for (int rowoffset = 0; rowoffset < invitations.length; rowoffset++) {
				...
				mData.setText(rowoffset, Column.CREATED.column, StringTools.formatDateAppleLike(new Date(invitation.getCreatedDate()), true));
				...
				if (createdLocally) {
					...
					if (invitation.getMaxAge() > 0) {
						...
						expires = StringTools.formatDateAppleLike(new Date(invitation.getCreatedDate() + invitation.getMaxAge()), true);
					} else {
						expires = "never";
					}
	...
	static class InvitationDialog extends OneSwarmDialogBox {
		...
		public void onClick(ClickEvent event) {
			if (event.getSource().equals(cancelButton)) {
				...
				if (Window.confirm(msg.add_friends_invite_view_confirm_delete_single())) {
					...
					l.add(invitation);
					OneSwarmRPCClient.getService().deleteFriendInvitations(OneSwarmRPCClient.getSessionID(), l, new AsyncCallback<Void>() {
						public void onFailure(Throwable caught) {
		...
		class InvitationDetailsPanel extends VerticalPanel {
			...
			public InvitationDetailsPanel(FriendInvitationLite invitation) {
				 ...
				 * expires
				 */
				expiresPicker = new DatePickerTextBox(new Date(invitation.getCreatedDate() + invitation.getMaxAge()));
			...
			public void save() {
				...
				invitation.setMaxAge(Math.max(0, expiresPicker.getValue().getTime() - invitation.getCreatedDate()));
				OneSwarmRPCClient.getService().updateFriendInvitations(OneSwarmRPCClient.getSessionID(), invitation, new AsyncCallback<Void>() {
					public void onFailure(Throwable caught) {
						caught.printStackTrace();
					}
