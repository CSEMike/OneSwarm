...
public class PermissionsGroup implements IsSerializable {
	...
	/**
	 * These need to be kept in sync with the GroupBean/SwarmBean classes in the core
	 */
	...
	public PermissionsGroup( String inGroupName, String [] inFriendKeys, boolean inUserGroup, long inID )
	{
		mMembershipKeys = new LinkedList<String>();
	...
	public void removeUser( String inKey )
	{
		if( isSpecial() )
		{	
			System.err.println("trying to remove a key to special user: " + inKey  + " from " + mGroupName);
		}
		else
		{
			if( mMembershipKeys.remove(inKey) == false )
			{
				System.err.println("tried to remove nonexistent key from group: " + inKey + " from " + mGroupName);
			}
		}
	}
