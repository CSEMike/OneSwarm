...
public class F2FInterface {
					...
					try {
						...
						while (f2fIpc == null) {
							...
							if (f2fIf != null && f2fIf.isOperational()) {
								...
								Debug.out("Friend to friend plugin not available, trynum=" + tryNum + " failed! " + f2fIf);
							}
							tryNum++;
						}
	 ...
	 * @return
	 */
	public List<FileListLite> getFileList(int connectionId, String filter, int startNum, int num, long maxCacheAge) {
		...
		if (f2fIpc != null) {
			try {
				f2fIpc.invoke("sendFileListRequest", new Object[] { connectionId, maxCacheAge, callback });
			} catch (IPCException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				return null;
		...
		try {
			...
			if (filter.equals("")) {
				...
				System.out.println("filtering for '" + filter + "' got " + filtered.getFileNum() + " matches");
			}
	...
	private List<FileListLite> groupAwareSubList(List<FileListLite> inRawFilesList, int inStartGroup, int inMaxGroups) {
		int skippedGroups = 0;
	...
	private static List<FileListLite> convertFileList(FileList fileList) {
		...
		for (FileCollection collection : fileList.getElements()) {
			...
			for (FileListFile file : files) {
				FileListLite fll = new FileListLite(collectionId, collectionName, file.getFileName(), file.getLength(), collection.getFileNum(), collection.getAddedTimeUTC(), totalGroups, false, false);
	...
	public void getMetaInfo(int connectionId, int channelId, String torrentId, int lengthHint, PluginCallback<byte[]> callback) {
		if (f2fIpc != null) {
			try {
				// if the length of the infohash is known we can speed up things
				// one rtt, -1 means not known
				f2fIpc.invoke("sendMetaInfoRequest", new Object[] { connectionId, channelId, Base64.decode(torrentId), lengthHint, callback });
			} catch (IPCException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	public List<TextSearchResult> getSearchResult(int searchId) {
		if (f2fIpc != null) {
			try {
				return (List<TextSearchResult>) f2fIpc.invoke("getTextSearchResult", new Object[] { searchId });
			} catch (IPCException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	@SuppressWarnings("unchecked")
	public FriendInfoLite[] getNewUsersFromXMPP(String xmppNetworkName, String username, char[] password, String machineName) throws Exception {

		List<Friend> friends = (List<Friend>) f2fIpc.invoke("getNewUsersFromXMPP", new Object[] { xmppNetworkName, username, password, machineName });
		if (friends == null) {
			return null;
		}
	...
	public void setFriendSettings(String publicKey, String nickname, boolean blocked, boolean canSeeFileList, boolean allowChat, boolean requestFileList, String group) {
		...
		System.out.println(publicKey + " " + nickname + " " + blocked + " " + canSeeFileList + " " + allowChat);
		if (f2fIpc != null) {
			try {
				f2fIpc.invoke("setFriendSettings", new Object[] { publicKey, nickname, blocked, canSeeFileList, allowChat, requestFileList, group });
			} catch (IPCException e) {
				e.printStackTrace();
			}
		}
	...
	public Map<Friend, FileList> getOnlineFileLists() {
		if (f2fIpc != null) {
			try {
				Map<Friend, FileList> f = (Map<Friend, FileList>) f2fIpc.invoke("getOnlineFileLists", new Object[0]);
				return f;
			} catch (IPCException e) {
				e.printStackTrace();
			}
	...
	public FriendInfoLite[] getLanOneSwarmUsers() {
		try {
			List<Friend> friends = (List<Friend>) f2fIpc.invoke("getLanOneSwarmUsers", new Object[] {});
			if (friends == null) {
				return null;
			}
			FriendInfoLite[] fLite = new FriendInfoLite[friends.size()];
	...
	public HashMap<String, String> getDeniedIncomingConnections() {
		try {
			HashMap<byte[], String> connections = (HashMap<byte[], String>) f2fIpc.invoke("getDeniedIncomingConnections", new Object[] {});
	...
	public Map<String, Integer> getNewFriendsCountsFromAutoCheck() {
		try {
			Map<String, Integer> counts = (Map<String, Integer>) f2fIpc.invoke("getNewFriendsCountsFromAutoCheck", new Object[] {});
			return counts;
		} catch (IPCException e) {
			e.printStackTrace();
	...
	public FriendInvitation createInvitation(String name, boolean canSeeFileList, long maxAge, byte securityLevel) {
		try {
			return (FriendInvitation) f2fIpc.invoke("createInvitation", new Object[] { name, canSeeFileList, maxAge, securityLevel });
		} catch (IPCException e) {
			e.printStackTrace();
			return null;
		}
	...
	public List<FriendInvitation> getLocallyCreatedInvitations() {
		try {
			return (List<FriendInvitation>) f2fIpc.invoke("getLocallyCreatedInvitations", new Object[] {});
		} catch (IPCException e) {
			e.printStackTrace();
	...
	public List<FriendInvitation> getRedeemedInvitations() {
		try {
			return (List<FriendInvitation>) f2fIpc.invoke("getRedeemedInvitations", new Object[] {});
		} catch (IPCException e) {
			e.printStackTrace();
			return new LinkedList<FriendInvitation>();
		}
	...
	public HashMap<String, Double> getSpeedCheckResult(int checkId) {
		try {
			return (HashMap<String, Double>) f2fIpc.invoke("getSpeedCheckResult", new Object[] { checkId });
		} catch (IPCException e) {
			e.printStackTrace();
