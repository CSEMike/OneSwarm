...
public class FriendPropertiesPanel extends Composite implements Updateable {
    ...
    private final CheckBox requestFileListBox = new CheckBox(
            msg.friend_properties_request_file_list());
    ...
    private final LabelWithHelp statusLabel = new LabelWithHelp("",
            msg.friend_properties_status_connect_help_HTML(), true);

    private boolean useDebug = false;

    final VerticalPanel mainPanel = new VerticalPanel();

    public FriendPropertiesPanel(final FriendInfoLite friend, final OneSwarmDialogBox parent,
            boolean useDebug) {
        ...
        if (friend.getStatus() == FriendInfoLite.STATUS_ONLINE
                || friend.getStatus() == FriendInfoLite.STATUS_HANDSHAKING) {
    ...
    private HorizontalPanel createPDeleteUndeletePanel(final OneSwarmDialogBox parent) {
        ...
        pdelete.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                boolean yes = Window.confirm(msg.friend_properties_confirm_perm_delete(friend
                        .getName()));
                if (yes) {
                    ...
                    OneSwarmRPCClient.getService().deleteFriends(OneSwarmRPCClient.getSessionID(),
                            new FriendInfoLite[] { friend }, new AsyncCallback<Void>() {
                                public void onFailure(Throwable caught) {
                                    Window.alert("got error when deleting friend: "
                                            + caught.getMessage());
                                }
    ...
    private HorizontalPanel createSettingsPanel() {
        ...
        HelpButton requestHelpButton = new HelpButton(
                msg.friend_properties_request_file_list_help());
    ...
    private void save(final OneSwarmDialogBox parent) {
        ...
        OneSwarmRPCClient.getService().setRecentChanges(OneSwarmRPCClient.getSessionID(), true,
                new AsyncCallback<Void>() {
                    public void onFailure(Throwable caught) {
                        OneSwarmGWT.log("delete friend: got error");
                    }
    ...
    public void saveChanges(final OneSwarmDialogBox parent, boolean forceUpdate,
            boolean reallySkipSkipped) {
    ...
    class MessageLog extends OneSwarmDialogBox {
        ...
        private void update() {
            OneSwarmRPCClient.getService().getDebugMessageLog(OneSwarmRPCClient.getSessionID(),
                    friend.getPublicKey(), new AsyncCallback<String>() {
                        public void onFailure(Throwable caught) {
                        }
    ...
    static class ConnectInfoPanel extends VerticalPanel {
        ...
        private static final String CONNECTED_SINCE_STRING = msg
                .friend_properties_connected_since();
        ...
        public ConnectInfoPanel(FriendInfoLite _friend) {
            ...
            connectButton.addClickHandler(new ClickHandler() {
                public void onClick(ClickEvent event) {
                    ...
                    service.connectToFriends(session, new FriendInfoLite[] { friend },
                            new AsyncCallback<Void>() {
                                public void onFailure(Throwable caught) {
            ...
            AsyncCallback<FriendInfoLite> callback = new AsyncCallback<FriendInfoLite>() {
                ...
                public void onSuccess(FriendInfoLite result) {
                    ...
                    if (lastConnectedDate != null) {
                        ...
                        updateGrid(2, 1,
                                friend.getLastConnectIp() + ":" + friend.getLastConnectPort());
                    } else {
