...
public class FriendListPanel extends VerticalPanel implements Updateable {
	...
	private final DisclosurePanel disclosurePanel = new DisclosurePanel(msg.friends_sidebar_header(), false);
	...
	public FriendListPanel() {
		...
		disclosurePanel.addCloseHandler(new CloseHandler<DisclosurePanel>() {
			public void onClose(CloseEvent<DisclosurePanel> event) {
				Cookies.setCookie("friend_list_open", false + "", OneSwarmConstants.TEN_YEARS_FROM_NOW);
				updateHeader();
			}
		});
		boolean open = true;
		...
		showOfflineItem = new MenuItem(showDisconnected == true ? hideOffline : showOffline, new Command() {
			public void execute() {
				showDisconnected = !showDisconnected;
				if (showDisconnected == true) {
					Cookies.setCookie(COOKIE_SHOW_OFFLINE_FRIENDS, "1", new Date(System.currentTimeMillis() + (1 * 365 * 24 * 60 * 60 * 1000)));
				} else {
					Cookies.setCookie(COOKIE_SHOW_OFFLINE_FRIENDS, "0", new Date(System.currentTimeMillis() + (1 * 365 * 24 * 60 * 60 * 1000)));
				}
		...
		showBlockedItem = new MenuItem(showBlocked == true ? hideBlockedStr : showBlockedStr, new Command() {
			public void execute() {
		...
		MenuItem optionsItem = new MenuItem("Options<img src='images/icons/disclosure.png' width='10px' height='10px'>", true, optionsMenu);
	...
	public void update(int count) {
		if (count % UPDATE_LIMIT == 0) {
			...
			if (System.currentTimeMillis() > nextNewMessageRPCCheck) {
				...
				service.getUnreadMessageCounts(session, new AsyncCallback<HashMap<String, Integer>>() {
					...
					public void onSuccess(HashMap<String, Integer> result) {
						...
						EntireUIRoot.getRoot(FriendListPanel.this).setUnreadChatCount(total);
						nextNewMessageRPCCheck = System.currentTimeMillis() + 2000;
					}
				});
			}

			service.getFriends(session, prevListId, showDisconnected, showBlocked, new AsyncCallback<FriendList>() {
				public void onFailure(Throwable caught) {
					// well, do nothing...
				...
				public void onSuccess(FriendList result) {
					...
					if (prevListId != result.getListId()) {
						OneSwarmGWT.log("updating friend list: " + (prevListId != result.getListId()) + " prev=" + prevListId + " next=" + result.getListId());
			...
			service.getNewFriendsCountsFromAutoCheck(session, new AsyncCallback<HashMap<String, Integer>>() {
				public void onFailure(Throwable caught) {
					OneSwarmGWT.log("error " + caught.getMessage());
				}
				...
				public void onSuccess(HashMap<String, Integer> result) {
					...
					if (sum > 0) {
						friendRequestsPanel.setHTML(msg.friends_sidebar_notice_friend_updates_HTML(sum));
	...
	private void updateHeader() {
		if (!disclosurePanel.isOpen()) {
			disclosurePanel.getHeaderTextAccessor().setText(msg.friends_sidebar_header() + " (" + oldFriendsList.length + ")");
		} else {
	...
	private class FriendPanel extends FocusPanel {
		...
		protected void updateChatCount() {
			if (mUnreadChatCounts.containsKey(friendInfoLite.getPublicKey())) {
				...
				nameLabel.setText(pre + StringTools.truncate(friendInfoLite.getName(), FriendPanel.MAX_LABEL_NAME_LENGTH - pre.length(), false));
			} else {
				if (statusImage.getUrl().endsWith(ImageConstants.ICON_CHAT)) {
					nameLabel.setText(StringTools.truncate(friendInfoLite.getName(), FriendPanel.MAX_LABEL_NAME_LENGTH, false));
		...
		private Image createEditImage() {
			...
			editImage.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					FriendPropertiesDialog dlg = new FriendPropertiesDialog(FriendListPanel.this, friendInfoLite, OneSwarmGWT.hasDevUpdates());
					dlg.show();
					dlg.setVisible(false);
					dlg.center();
		...
		private void setImageSettings(HorizontalPanel panel, Image i, int imageWidth, int imageHeigth) {
			panel.add(i);
		...
		private void updateName(FriendInfoLite f) {
			...
			nameLabel.setTitle(name + (this.friendInfoLite.isCanSeeFileList() == false ? " (limited)" : ""));
		...
		private void updateStatus(FriendInfoLite f, boolean updateChat) {
			...
			if (f.getStatus() != oldStatus) {
				...
				if (f.getStatus() == FriendInfoLite.STATUS_ONLINE) {
					...
					statusImage.setTitle("Connected" + (f.isCanSeeFileList() == false ? " (limited)" : ""));
				} else {
