...
public class MagicDirectorySettingsPanel extends SettingsPanel implements ClickHandler {
	...
	public MagicDirectorySettingsPanel() {
		...
		OneSwarmRPCClient.getService().getIntegerParameterValue(OneSwarmRPCClient.getSessionID(), "oneswarm.watchdir.refresh.interval", 
				...
				public void onSuccess(Integer result) {
					...
					if( result.intValue() == 0 ) {
						...
						try {
							...
							if( !found ) { 
								timerListBox.addItem(result + " " + msg.settings_files_minutes());
...
//		System.out.println("get settings rpc");
		OneSwarmRPCClient.getService().getStringListParameterValue(OneSwarmRPCClient.getSessionID(), "Magic Watch Directories", new AsyncCallback<ArrayList<String>>() {
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
...
//				System.out.println("got watch dir result");
	...
	public void sync() {
		...
		OneSwarmRPCClient.getService().setStringListParameterValue(OneSwarmRPCClient.getSessionID(), "Magic Watch Directories", converted, new AsyncCallback<Void>() {
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
		...
		OneSwarmRPCClient.getService().setIntegerParameterValue(OneSwarmRPCClient.getSessionID(), "oneswarm.watchdir.refresh.interval", refreshInterval, new AsyncCallback<Void>(){
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
	...
	public void onClick(ClickEvent event) {
		...
		if (sender.equals(addButton)) {
			...
			OneSwarmRPCClient.getService().selectFileOrDirectory(OneSwarmRPCClient.getSessionID(), true, new AsyncCallback<String>() {
				...
				public void onSuccess(final String selectedPath) {
					...
					if (watchDirs.contains(selectedPath) == false) {
						MagicTypeSelectionDialog dlg = new MagicTypeSelectionDialog(MagicWatchType.Magic, new MagicTypeSelectionDialog.Callback() {
							public void done(boolean cancelled, MagicWatchType which) {
								if (!cancelled) {
									MagicPath mpath = new MagicPath(selectedPath, which);
									watchDirs.add(mpath);
									refreshList();
								}
							}
			...
			MagicTypeSelectionDialog dlg = new MagicTypeSelectionDialog(watchDirs.get(currentList.getSelectedIndex()).mType, new MagicTypeSelectionDialog.Callback() {
				public void done(boolean cancelled, MagicWatchType which) {
					if (!cancelled) {
