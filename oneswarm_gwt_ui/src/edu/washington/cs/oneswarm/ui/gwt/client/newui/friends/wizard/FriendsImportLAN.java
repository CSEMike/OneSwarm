...
public class FriendsImportLAN extends VerticalPanel implements Updateable {
	...
	private void addToIgnoreList(final FriendInfoLite friendInfo) {
		OneSwarmRPCClient.getService().addToIgnoreRequestList(OneSwarmRPCClient.getSessionID(), friendInfo, new AsyncCallback<Void>() {
			public void onFailure(Throwable caught) {
				Window.alert("got error when denying friend request friend: " + caught.getMessage());
			}
	...
	private VerticalPanel getMyInfoPanel() {
		...
		nameEditAnchor.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				ComputerNameDialog dlg = new ComputerNameDialog(nameLabel.getText(), true, new ComputerNameDialog.NameSetCallback() {
		...
		OneSwarmRPCClient.getService().getSelf(OneSwarmRPCClient.getSessionID(), new AsyncCallback<FriendInfoLite>() {
			public void onFailure(Throwable caught) {
				Window.alert("got error when denying friend request friend: " + caught.getMessage());
			}
	...
	private class LanFriendPanel extends HorizontalPanel {
		...
		public void update(final FriendInfoLite f) {
			if (!f.getPublicKey().equals(friendInfoLite.getPublicKey())) {
				throw new RuntimeException("tried to update LanFriendPanel, but publickey does no match");
			}
			if (!isFriend) {
				checkBox.setText(f.getName());
			}
			...
			service.addFriend(session, f, testOnly, new AsyncCallback<Void>() {
				...
				public void onSuccess(Void result) {
					...
					service.getDeniedIncomingConnections(session, new AsyncCallback<HashMap<String, String>>() {
						public void onFailure(Throwable caught) {
						}
