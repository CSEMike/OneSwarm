...
public class FriendsImportWizard extends OneSwarmDialogBox {
	...
	private final static String[] IMPORT_SOURCES = { FRIEND_SRC_COMMUNITY, FRIEND_SRC_GTALK, FRIEND_SRC_LAN, FRIEND_SRC_MANUAL, FRIEND_SRC_INVITATIONS };
	private final static String[] IMPORT_SOURCES_KEYS = { FriendsImportCommunityServer.FRIEND_NETWORK_COMMUNITY_NAME, FriendsImportGtalk.FRIEND_NETWORK_GTALK_NAME, FriendsImportLAN.FRIEND_NETWORK_LAN_NAME, "", "Invitation" };
	...
	private FriendsImportCallback friendWizardCallback = new FriendsImportCallback() {
		...
		public void connectSuccesful(FriendInfoLite[] changes, boolean showSkip) {
			final FriendPropertiesTable friendPropertiesTable = new FriendPropertiesTable(changes, true, showSkip);
			panel.clear();
				...
				public void onClick(ClickEvent event) {
					if (friendPropertiesTable != null) {
						...
						if (removal != null) {
							if (removal.getValue()) {
								if (friendsToDelete.size() > 0) {
									OneSwarmRPCClient.getService().deleteFriends(OneSwarmRPCClient.getSessionID(), friendsToDelete.toArray(new FriendInfoLite[0]), new AsyncCallback<Void>() {
										public void onFailure(Throwable caught) {
											caught.printStackTrace();
										}

										public void onSuccess(Void result) {
											System.out.println("deletes successful: " + friendsToDelete.size());
	...
	protected void show_community_import(final CommunityRecord toAdd) {
		...
		if (newFriendRequestCounts.containsKey(FriendsImportCommunityServer.FRIEND_NETWORK_COMMUNITY_NAME)) {
			isNotification = newFriendRequestCounts.get(FriendsImportCommunityServer.FRIEND_NETWORK_COMMUNITY_NAME) > 0;
		}

		VerticalPanel communityP = new FriendsImportCommunityServer(friendWizardCallback, isNotification, toAdd);
		panel.clear();
