...
public class F2FSearchQueryWithResults extends HorizontalPanel implements Updateable {
    ...
    public F2FSearchQueryWithResults(String keywords, final EntireUIRoot inRoot, int searchId,
            HTML statusLabel) {
        this(keywords, inRoot, searchId, statusLabel, null);
    }

    public F2FSearchQueryWithResults(String keywords, final EntireUIRoot inRoot, int searchId,
            HTML statusLabel, ResultsCallback callback) {
        ...
        resultsTable = new ScrollTable(new FixedWidthGrid(0, Strings.F2F_RESULTS_COLS.length),
        ...
        mResultsData.addTableListener(new TableListener() {
            public void onCellClicked(SourcesTableEvents arg0, int row, int arg2) {
                ...
                TextSearchResultLite s = ((TextSearchResultLiteAwareRow) mResultsData.getWidget(
                        row, 0)).getSearchResult();
                if (s.isInLibrary()) {
                    UpdateSkippedFilesDialog dlg = new UpdateSkippedFilesDialog(EntireUIRoot
                            .getRoot(F2FSearchQueryWithResults.this), s.getTorrentInfo());
                    ...
                    FileListLite clickedRow = new FileListLite(s.getCollectionId(), s
                            .getCollectionName(), s.getFileName(), s.getFileSize(), 0, s
                            .getAddedTimeUTC(), 0, false, false);
                    OneSwarmDialogBox dlg;
                    if (age > SEARCH_EXPIRE_TIME) {
                        dlg = new SearchExpiredDialog(clickedRow);
                    } else {
                        dlg = new TorrentDownloaderDialog(inRoot, null, s.getConnectionId(), s
                                .getChannelId(), 0, clickedRow, false, s.getThroughFriends(), s
                                .getFriendDelay());
                    }
                    dlg.show();
                    dlg.setVisible(false);
    ...
    public void update(int count) {
        ...
        service.getIntegerParameterValue(session, "oneswarm.max.ui.search.results",
            ...
            public void onSuccess(TextSearchResultLite[] result) {
                ...
                if (mStatusLabel != null) {
                    ...
                    if (result.length >= maxSearchResults) {
                        newStatusHtml = ("Search completed, got more than " + maxSearchResults
                                + " responses, showing 1-" + result.length + " (<a href='#'>change limit</a>)");
                        stopUpdates();
                        mStatusLabel.addClickHandler(new ClickHandler() {

                            public void onClick(ClickEvent event) {
                                final EntireUIRoot root = EntireUIRoot
                                        .getRoot(F2FSearchQueryWithResults.this);
                                SettingsDialog dlg = new SettingsDialog(root,
                                        root.swarmFileBrowser, 2);
                            }
                        });

                    } else if (fakeDone == 100) {
                        if (result.length == 0) {
                            newStatusHtml = ("Search completed, no results found matching '"
                                    + keywords + "'");
                            stopUpdates();
                        } else {
                            newStatusHtml = ("Search completed, " + result.length
                                    + " results found matching '" + keywords + "'");
                            stopUpdates();
                        }
                    } else {
        ...
        public SearchExpiredDialog(final FileListLite clickedRow) {
            ...
            searchButton.addClickListener(new ClickListener() {
                public void onClick(Widget sender) {
                    ...
                    EntireUIRoot.getRoot(F2FSearchQueryWithResults.this).displaySearch(
                            fileName.substring(0, Math.min(fileName.length(), 100)) + " id:"
                                    + clickedRow.getCollectionId());
                    SearchExpiredDialog.this.hide();
                }
            });
