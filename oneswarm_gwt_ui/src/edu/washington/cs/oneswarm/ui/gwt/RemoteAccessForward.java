...
public class RemoteAccessForward {
	...
	public synchronized void start() {
		if (!running) {
			running = true;
			String type = COConfigurationManager.getStringParameter(OneSwarmConstants.REMOTE_ACCESS_LIMIT_TYPE_KEY, OneSwarmConstants.REMOTE_ACCESS_LIMIT_TYPE_NOLIMIT);
			String filter = COConfigurationManager.getStringParameter(OneSwarmConstants.REMOTE_ACCESS_LIMIT_IPS_KEY, "");
			try {
				NetworkManager.getSingleton().requestIncomingConnectionRouting(matcher, new OsNetworkRouterListener(type, filter), new DummyMessageDecoderFactory());
	...
	private class ConnectionForwarder {
		...
		public ConnectionForwarder(NetworkConnection connection) {
			...
			this.remoteIp = connection.getEndpoint().getNotionalAddress().getAddress().getHostAddress();
		...
		public void stop(boolean deregister) {
			if (!quit) {
				logger.fine("forwarder stopped, ->ssl: " + sslToGwtWeb.getTotalWritten() + " ->gwt: " + gwtWebToSSL.getTotalWritten());
			}
			this.quit = true;
	...
	private static class HttpSSLMatcher implements NetworkManager.ByteMatcher {
		...
		public Object matches(TransportHelper transport, ByteBuffer to_compare, int port) {
			...
			boolean matches = compareString.startsWith(GET_HANDSHAKE) || compareString.startsWith(POST_HANDSHAKE);
			if (matches) {
	...
	private class OsNetworkRouterListener implements NetworkManager.RoutingListener {
		...
		public OsNetworkRouterListener(String ipFilterType, String ipFilterString) throws OneSwarmException {
		...
		public void connectionRouted(NetworkConnection connection, Object routing_data) {
			...
			if (!connection.getTransport().getEncryption().startsWith(OneSwarmSslTransportHelperFilterStream.SSL_NAME)) {
				logger.fine("remote OSGWT ui connection without SSL: " + connection + " (" + connection.getTransport().getEncryption() + "), redirecting");
				try {
			...
			logger.fine("Incoming connection from [" + connection + "] successfully routed to OneSwarm GWT Remote: encr: " + connection.getTransport().getEncryption());
			try {
		...
		for (String s : split) {
			...
			if (s.contains("-")) {
				...
				try {
					...
					throw new OneSwarmException(baseProblem + " unable to parse ip: '" + ip + "':" + e.getMessage());
				}
				...
				try {
					...
					throw new OneSwarmException(baseProblem + " unable to parse ip: '" + parts[0] + "':" + e.getMessage());
				}
				...
				try {
					...
					throw new OneSwarmException(baseProblem + " unable to parse ip: '" + s + "':" + e.getMessage());
				}
				IpRange r = new IpRangeImpl("", baseIp.getHostAddress(), baseIp.getHostAddress(), false);
				allowedRanges.add(r);
			}
		}
		return allowedRanges;
	...
	private static boolean isInRange(List<IpRange> ranges, String ip) {
		for (IpRange ipRange : ranges) {
			if (ipRange.isInRange(ip)) {
				logger.finer(ip + " is in range: " + ipRange.getStartIp() + "-" + ipRange.getEndIp());
				return true;
			}
		}
