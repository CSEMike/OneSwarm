...
public class StatelessSwarmFilter {
	...
	enum SortMetric {
		Name(new Comparator<DownloadManager>() {
			public int compare(DownloadManager lhs, DownloadManager rhs) {
				return (new String(lhs.getTorrent().getName()).toLowerCase().compareTo((new String(rhs.getTorrent().getName())).toLowerCase()));
			}
			...
			public int compare(DownloadManager lhs, DownloadManager rhs) {
				 ...
				 */
				long lhs_time = lhs.getData("friend-added-time") != null ? (Long) lhs.getData("friend-added-time") : lhs.getDownloadState().getLongParameter(DownloadManagerState.PARAM_DOWNLOAD_ADDED_TIME);
	...
	private List<DownloadManager> getSingleFriendsFiles(int selectedFriendID) {
		...
		for (FileCollection collection : fileList.getElements()) {
			 ...
			 */
			DownloadManager ours = AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManager(new HashWrapper(Base64.decode(collection.getUniqueID())));
			if (ours != null) {
				/**
				...
				if (filteredUntilDeleteBase64.contains(base64Hash)) {
					DownloadManagerAdapter adapter = new DownloadManagerAdapter(collection, friend.getConnectionId(), friend.getNick());
					out.add(adapter);
				} else {
					out.add(ours);
	...
	private static boolean deepKeywordMatch(DownloadManager d, String keyword) {
		...
		if (d.getDisplayName().toLowerCase().contains(keyword) == false) {
			 ...
			 */
			String album = d.getDownloadState().getAttribute(FileCollection.ONESWARM_ALBUM_ATTRIBUTE);
			if (album != null) {
			...
			String artist = d.getDownloadState().getAttribute(FileCollection.ONESWARM_ARTIST_ATTRIBUTE);
			if (artist != null) {
	...
	public FilteredSwarmInfo filterSwarms(String[] inKeywords, SortMetric inSortingMetric, FileTypeFilter inFileType, boolean includeF2F, int selectedFriendID, String inTagPath) {
		...
		if (selectedFriendID != Integer.MIN_VALUE) {
			 ...
			 */
			for (DownloadManager d : (List<DownloadManager>) AzureusCoreImpl.getSingleton().getGlobalManager().getDownloadManagers()) {
				if (d.getTorrent() == null) {
					logger.warning("null torrent!: " + d.getDisplayName());
					continue;
				}
				if (d.getDownloadState().getBooleanAttribute(Sha1DownloadManager.ONESWARM_AUTO_ADDED)) {
					continue;
				}
			...
			if (includeF2F) {
				...
				for (Friend friend : mCore.getF2FInterface().getOnlineFileLists().keySet()) {
					...
					for (FileCollection collection : fileList.getElements()) {
						 ...
						 * we have locally
						 */
						if (friendHashes.contains(collection.getUniqueID()) || ourHashes.contains(collection.getUniqueID())) {
							continue;
						}
		...
		logger.fine("keyword time: " + keyword_time + " total: " + (System.currentTimeMillis() - start));
	...
	private boolean matchTags(DownloadManager d, String inTagPath) {
		...
		String[] tags = d.getDownloadState().getListAttribute(FileCollection.ONESWARM_TAGS_ATTRIBUTE);
	...
	public static FileTree getTagsFromSwarms(List<DownloadManager> outSwarms) {
		try {
			...
			for (org.gudy.azureus2.core3.download.DownloadManager dm : outSwarms) {
				String[] tags_raw = dm.getDownloadState().getListAttribute(FileCollection.ONESWARM_TAGS_ATTRIBUTE);
				if (tags_raw == null) {
					continue;
				}
