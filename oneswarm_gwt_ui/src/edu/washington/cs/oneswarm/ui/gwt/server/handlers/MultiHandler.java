...
public class MultiHandler extends ServletHandler {
	...
	public void handle(String target, HttpServletRequest request, HttpServletResponse response, int dispatch) throws IOException, ServletException {
		...
		if (target.startsWith("/stream")) {
			...
			logger.fine("file=" + request.getParameter("file") + " start=" + request.getParameter("start"));
		}
		// TODO we should change the /image to something more random...
		if ((target.startsWith(OneSwarmConstants.servletPath) && hasCookie) || (target.startsWith("/oneswarmgwt/image") && hasCookie && !target.startsWith("/oneswarmgwt/images/"))) {
		...
		} else if (target.startsWith(OneSwarmConstants.BROWSE_SHARE_PATH) || target.startsWith(OneSwarmConstants.DOWNLOAD_SHARE_PATH)) {
	...
	private void initializeServletHandler() {
		...
		super.addServletWithMapping(new ServletHolder(new OneSwarmUIServiceImpl(coreInterface, remote)), OneSwarmConstants.servletPath);
		super.addServletWithMapping(new ServletHolder((new PreviewImageGenerator())), "/oneswarmgwt/image");
	...
	private void securityCheck(Request request, Response response) throws IOException {
		...
		if (!OsgwtuiMain.LOCALHOST.equals(hostHeader)) {
			throw new IOException("request not for " + OsgwtuiMain.LOCALHOST + ", dropping connection");
		}
