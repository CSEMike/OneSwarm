...
public class NewUserSetupPanel extends VerticalPanel implements Updateable {
	...
	enum State { 
		kLoading, 
		kNoFriendsNoCommunity, 
		kNoFriends,
		kNoOnlineFriends, 
		kFriendsButNotOnline, 
		kFriendsOnline
	}
...
//		HTML w = new HTML("<div id=\"" + OneSwarmCss.CSS_NOTHING_SHOWING + "\">" + Strings.get(Strings.NO_FILES_MESSAGE) + "</div>");
//		this.add(w);
//		
	...
	private void defaultClicked() { 
		...
		OneSwarmRPCClient.getService().applyDefaultSettings(OneSwarmRPCClient.getSessionID(), new AsyncCallback<Void>(){
...
//			System.out.println("new user rpc");
			...
			OneSwarmRPCClient.getService().getNumberOnlineFriends(OneSwarmRPCClient.getSessionID(), new AsyncCallback<Integer>(){
				public void onFailure(Throwable caught) {
...
//					System.out.println("online friends: " + onlineFriends); 
					...
					if( onlineFriends == 0 ) {
						...
						OneSwarmRPCClient.getService().getNumberFriendsCount(OneSwarmRPCClient.getSessionID(), new AsyncCallback<Integer>(){
							public void onFailure(Throwable caught) {
								caught.printStackTrace();
							}
...
//								System.out.println("total friends: " + anyfriends);
								
								if( anyfriends == 0 ) {
									
//									System.out.println("0 total friends");
									
									// now also check if there are no community servers
									mNextRPC = System.currentTimeMillis() + 10*1000;
									OneSwarmRPCClient.getService().getStringListParameterValue(OneSwarmRPCClient.getSessionID(), "oneswarm.community.servers", new AsyncCallback<ArrayList<String>>(){
										...
										public void onSuccess(ArrayList<String> result) {
											if( result.size() == 0 ) { 
												System.out.println("no community servers");
