...
class PortsSettingsPanel extends SettingsPanel implements Updateable {
    ...
    public PortsSettingsPanel() {
        ...
        natCheck.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                ...
                OneSwarmRPCClient.getService().setIntegerParameterValue(
                        OneSwarmRPCClient.getSessionID(), "TCP.Listen.Port",
                        Integer.parseInt(tcp_port_box.getText()), new AsyncCallback<Void>() {
                            ...
                            public void onSuccess(Void result) {
                                 ...
                                 * trigger the check
                                 */
                                OneSwarmRPCClient.getService().triggerNatCheck(
                                        ...
                                        new AsyncCallback<Void>() {
                                            ...
                                            public void onSuccess(Void result) {
                                                OneSwarmGWT
                                                        .addToUpdateTask(PortsSettingsPanel.this);
                                            }
                                        });
                            }
                        });
        ...
        OneSwarmRPCClient.getService().getIntegerParameterValue(OneSwarmRPCClient.getSessionID(),
                "TCP.Listen.Port", new AsyncCallback<Integer>() {
                    public void onFailure(Throwable caught) {
                        caught.printStackTrace();
                    }
        ...
        OneSwarmRPCClient.getService().getIntegerParameterValue(OneSwarmRPCClient.getSessionID(),
                "UDP.Listen.Port", new AsyncCallback<Integer>() {
                    public void onFailure(Throwable caught) {
                        caught.printStackTrace();
                    }
    ...
    public void sync() {
        ...
        OneSwarmRPCClient.getService().setIntegerParameterValue(OneSwarmRPCClient.getSessionID(),
                "UDP.Listen.Port", Integer.parseInt(udp_port_box.getText()),
                new AsyncCallback<Void>() {
                    public void onFailure(Throwable caught) {
                        caught.printStackTrace();
                    }
        ...
        OneSwarmRPCClient.getService().setIntegerParameterValue(OneSwarmRPCClient.getSessionID(),
                "TCP.Listen.Port", Integer.parseInt(tcp_port_box.getText()),
                new AsyncCallback<Void>() {
                    public void onFailure(Throwable caught) {
                        caught.printStackTrace();
                    }
    ...
    public void update(int count) {
        OneSwarmRPCClient.getService().getNatCheckResult(OneSwarmRPCClient.getSessionID(),
                new AsyncCallback<HashMap<String, String>>() {
                    ...
                    public void onSuccess(HashMap<String, String> result) {
                        ...
                        if (status == null) {
                            ...
                            natCheck.setEnabled(false);
                            natStatusLabel.setText(PORT_STATUS
                                    + msg.settings_net_tcp_nat_test_status_ok());
                            natStatusLabel.setTitle(msg
                                    .settings_net_tcp_nat_test_status_ok_title(result.get("ip")
                                            + ":" + result.get("port")));
                            OneSwarmGWT.removeFromUpdateTask(PortsSettingsPanel.this);
                        } else if ("-1".equals(status)) {
                            natCheck.setEnabled(true);
                            natStatusLabel.setText(PORT_STATUS
                                    + msg.settings_net_tcp_nat_test_status_failed());
                            OneSwarmGWT.removeFromUpdateTask(PortsSettingsPanel.this);
                        } else if ("0".equals(status)) {
                            natCheck.setEnabled(false);
                            natStatusLabel.setText(PORT_STATUS
                                    + msg.settings_net_tcp_nat_test_status_waiting());
                        }
                    }
                });
    }
