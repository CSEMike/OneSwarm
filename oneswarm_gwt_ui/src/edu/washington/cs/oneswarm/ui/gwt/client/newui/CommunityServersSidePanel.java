...
public class CommunityServersSidePanel extends VerticalPanel implements Updateable {
	...
	class ServerPanel extends FocusPanel {
		...
		private void refreshUI() {
			String name = server.getServer_name() == null ? server.getUrl() : server.getServer_name();
			String labelName;
...
//			System.out.println(statusImage.getUrl() + " / " + server.getSplash_path());
	...
	private final DisclosurePanel disclosurePanel = new DisclosurePanel(msg.community_servers_sidebar_header(), false);
	...
	public CommunityServersSidePanel() {
		...
		MenuItem addFriendItem = new MenuItem(msg.community_servers_sidebar_add(), new Command() {
			public void execute() {
				OneSwarmDialogBox dlg = new FriendsImportWizard(FriendsImportWizard.FRIEND_SRC_COMMUNITY);
				dlg.show();
