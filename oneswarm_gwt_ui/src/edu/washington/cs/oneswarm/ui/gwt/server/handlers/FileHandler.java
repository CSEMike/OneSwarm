...
public class FileHandler extends AbstractHandler {
    ...
    static {
        File jarFile = new File(SystemProperties.getApplicationPath() + File.separator
                + "OneSwarmAzMods.jar");
    ...
    public FileHandler() {
        // If we can't find the resource in the current ClassLoader, we might be
        // the SWT target
        // in debug mode. Eclipse won't have built the GWT targets, but if we
        // built them
        // externally, retrieve from the gwt-dist directory.
        if (System.getProperty("debug.war") != null) {
            try {
                classLoader = new URLClassLoader(new java.net.URL[] { new File(
                        System.getProperty("debug.war")).toURI().toURL() }, getClass()
                        .getClassLoader());
    ...
    private boolean serve(String filename, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
         ...
         */
        if (fullPath.startsWith("images") || fullPath.equals("favicon.ico")
        ...
        if (useCache) {
            ...
            if (last_modified > 0) {
                ...
                logger.finest("setting modified tag, " + last_modified + " (prev was: "
                        + if_modified + ")");
            }
            ...
            if (filename.endsWith("nocache.js")) {
                ...
                response.setDateHeader(HttpHeaders.EXPIRES, System.currentTimeMillis()
                        + (oneYearInSeconds * 1000));
                response.setHeader(HttpHeaders.CACHE_CONTROL, "max-age=" + oneYearInSeconds
                        + ", public");
            } else if (filename.startsWith("oneswarmgwt/images/")
                    || filename.startsWith("gwt/standard")) {
    ...
    private void injectLocaleMetaTag(HttpServletRequest request, InputStream in, OutputStream out)
            throws IOException {
