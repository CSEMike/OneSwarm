...
public class SwarmsBrowser extends VerticalPanel implements Updateable {
	...
	HTML allFilesFiltered = new HTML("<div id=\"" + OneSwarmCss.CSS_NOTHING_SHOWING + "\">" + msg.swarm_browser_no_files_search_friends_HMTL("") + "</a></div>");
	HTML noFilesHTML = new HTML("<div id=\"" + OneSwarmCss.CSS_NOTHING_SHOWING + "\">" + Strings.get(Strings.NO_FILES_MESSAGE) + "</div>");
	HTML noFriendsFilesHTML = new HTML("<div id=\"" + OneSwarmCss.CSS_NOTHING_SHOWING + "\">" + msg.swarm_browser_no_files_shared() + "</div>");
	...
	enum DoubleClickAction {
		BrowserPlay(Strings.SWARM_BROWSER_PLAY), SystemPlay(Strings.SWARM_DEFAULT_PLAY), OpenFolder(Strings.SWARM_REVEAL);
	...
	public SwarmsBrowser(boolean useDebug) {
		...
		mSwarmsPerPagePanel.setCellVerticalAlignment(swarmsPerPagePopup, VerticalPanel.ALIGN_MIDDLE);
		...
		mSwarmsPerPage = Integer.parseInt(swarmsPerPagePopup.getItemText(swarmsPerPagePopup.getSelectedIndex()));
		swarmsPerPagePopup.addChangeListener(new ChangeListener() {
			public void onChange(Widget sender) {
				mSwarmsPerPage = Integer.parseInt(swarmsPerPagePopup.getItemText(swarmsPerPagePopup.getSelectedIndex()));
				Cookies.setCookie(COOKIE_SWARMS_PER_PAGE_INDEX, swarmsPerPagePopup.getSelectedIndex()+"", OneSwarmConstants.TEN_YEARS_FROM_NOW);
				pageZero();
		...
		mShowFriendsCheckbox.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				Cookies.setCookie(COOKIE_SHOW_FRIENDS_SWARMS, mShowFriendsCheckbox.getValue()+"", OneSwarmConstants.TEN_YEARS_FROM_NOW);
				refreshActive(true);
			}
		});
		...
		allFilesFiltered.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				if (mFilterPattern.trim().length() < NavigationFilterBar.MIN_SEARCH_LENGTH) {
					Window.alert(msg.swarm_browser_warning_short_search(NavigationFilterBar.MIN_SEARCH_LENGTH));
					return;
				}
	...
	private ChangeListener mJumpChangeListener = new ChangeListener() {
		public void onChange(Widget sender) {
			System.out.println("change listener, mJumpList.getSelectedIndex: " + mJumpList.getSelectedIndex());
	...
	private HorizontalPanel createPagingNavigationPanel() {
		...
		mSortByList.addChangeListener(new ChangeListener() {
			public void onChange(Widget sender) {
				...
				mSortingMetric = Integer.parseInt(mSortByList.getValue(mSortByList.getSelectedIndex()));
		...
		final Hyperlink viewLink = new Hyperlink(which.equals("icons") ? Strings.get(Strings.LIST_VIEW) : Strings.get(Strings.ICON_VIEW), "");
		viewLink.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				if (viewLink.getText().equals(Strings.get(Strings.LIST_VIEW))) {
					...
					mUsingIconView = false;
					Cookies.setCookie(COOKIE_SWARMS_VIEW, "list", OneSwarmConstants.TEN_YEARS_FROM_NOW);
					refreshActive(false);
				} else {
					viewLink.setText(Strings.get(Strings.LIST_VIEW));
					mUsingIconView = true;
					Cookies.setCookie(COOKIE_SWARMS_VIEW, "icons", OneSwarmConstants.TEN_YEARS_FROM_NOW);
					refreshActive(false);
				}
			}
		});
	...
	private void updateNavigationPanel() {
		...
		// selected
		// at start
		int count = (int) Math.max(1, Math.ceil((double) mTotalSwarmsForFilterType / (double) mSwarmsPerPage));
	...
	private void updateShowingLabel() {
		...
		mShowingLabel.setText(msg.swarm_browser_showing_swarms(curr, Math.min(curr + mSwarmsPerPage, mTotalSwarmsForFilterType), (mTotalSwarmsForFilterType)) + extra);
	}
	...
	public void dispatchSwarmAction(String inActionKey, TorrentInfo[] inTorrents) {
		...
		if (inActionKey.equals(Strings.SWARM_BROWSER_PLAY)) {
			...
			if (inTorrents[0].isF2FOnly() == false) {
				...
				if (inTorrents[0].getProgress() < 1000) {
					if (inTorrents[0].getDownloadRate() == 0 || (inTorrents[0].getSeeders() + inTorrents[0].getLeechers() == 0)) {
						goForIt = Window.confirm(msg.swarm_browser_warning_stalled_swarm());
					} else if (inTorrents[0].getDownloadRate() < 50 && inTorrents[0].getProgress() < 900) {
				...
				for (TorrentInfo inTorrent : inTorrents) {
					TorrentDownloaderDialog dlg = new TorrentDownloaderDialog(mEntireUIRoot, null, inTorrent.getF2F_ID(), inTorrent.getFileListLiteRepresentation(), true);
			...
			for (TorrentInfo inTorrent : inTorrents) {
				UpdateSkippedFilesDialog dlg = new UpdateSkippedFilesDialog(mEntireUIRoot, inTorrent);
			...
			TorrentInfo[] infos = getSelectedSwarms();
			if (infos != null && infos.length >= 1) {
				OneSwarmRPCClient.getService().copyTorrentInfoToMagnetLink(OneSwarmRPCClient.getSessionID(), new String[] { infos[0].getTorrentID() }, new AsyncCallback<String>() {
			...
			OneSwarmRPCClient.getService().selectFileOrDirectory(OneSwarmRPCClient.getSessionID(), false, new AsyncCallback<String>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}
				...
				public void onSuccess(String result) {
					...
					/**
					 * We enable everything here since it's pretty likely that
					 * what is being opened is a public torrent. (Even if it is
					 * an F2F torrent, it will have an invalid tracker URL)
					 */
					...
					OneSwarmRPCClient.getService().addDownloadFromLocalTorrentDefaultSaveLocation(OneSwarmRPCClient.getSessionID(), result, permitted, new AsyncCallback<Void>() {
						public void onFailure(Throwable caught) {
							caught.printStackTrace();
							// new ReportableErrorDialogBox(caught.getMessage(),
							// false);
							Window.alert(msg.swarm_browser_warning_invalid_torrent());
						}
			...
			if (OneSwarmGWT.isRemoteAccess()) {
				...
				for (TorrentInfo swarm : swarms) {
					Window.open("/files/" + swarm.getTorrentID().replace("urn_btih_", "") + "/", "_blank", "status=0,toolbar=0,menubar=0,location=0,resizable=0");
				}
			} else {
				/*
				 * otherwise open the normal folder view
				 */
				OneSwarmRPCClient.getService().revealSwarmInFinder(OneSwarmRPCClient.getSessionID(), getSelectedSwarms(), new AsyncCallback<ReportableException>() {
					public void onFailure(Throwable caught) {
						caught.printStackTrace();
					}
			...
			OneSwarmRPCClient.getService().openFileDefaultApp(OneSwarmRPCClient.getSessionID(), getSelectedSwarms(), new AsyncCallback<ReportableException>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}
			...
			if (mPreviousFullRefreshTotalSwarmsCount == 0) {
				...
				OneSwarmRPCClient.getService().getPagedAndFilteredSwarms(0, Integer.MAX_VALUE, "", SORT_BY_NAME_ID, "all", false, Integer.MIN_VALUE, null, new AsyncCallback<PagedTorrentInfo>() {
					public void onFailure(Throwable caught) {
						caught.printStackTrace();
					}
					...
					public void onSuccess(PagedTorrentInfo result) {
						...
						SwarmPermissionsDialog dlg = new SwarmPermissionsDialog(result.swarms, selected);
						dlg.show();
						dlg.setVisible(false);
						dlg.center();
						dlg.setPopupPosition(dlg.getPopupLeft(), Window.getScrollTop() + 100);
						dlg.setVisible(true);
					}
				});
			}
		...
		// }
			...
			if (selected[0].isStarted()) {
				...
				OneSwarmRPCClient.getService().stopTorrent(OneSwarmRPCClient.getSessionID(), selected_ids, new AsyncCallback<Boolean>() {
					public void onFailure(Throwable caught) {
						caught.printStackTrace();
					}
				...
				OneSwarmRPCClient.getService().startTorrent(OneSwarmRPCClient.getSessionID(), selected_ids, new AsyncCallback<Boolean>() {
					public void onFailure(Throwable caught) {
						caught.printStackTrace();
					}
	...
	private void selectSwarm(TorrentInfo inTorrent, boolean fromClick, boolean shiftDown) {
		if (inTorrent != null) {
			if (mSelectedSwarms.size() > 0) {
				if (mSelectedSwarms.contains(inTorrent) && fromClick == true && (System.currentTimeMillis() - mLastClickTime) < DOUBLE_CLICK_THRESHOLD) {
		...
		if (inTorrent != null) {
			...
			canvas.arc(width - 5, height - 5, 5, (float) (2 * Math.PI), (float) (5 * Math.PI / 2), false);
	...
	public void startBrowserPlayer(TorrentInfo inTorrent) {
		OneSwarmDialogBox dlg = new VideoDialog(inTorrent.getTorrentID(), inTorrent.getProgress() != 1000, "", inTorrent.getNumFiles(), mEntireUIRoot);
		dlg.show();
		dlg.setVisible(false);
		dlg.center();
	...
	protected void refreshActive(boolean pageZero) {
		...
		OneSwarmRPCClient.getService().getPagedAndFilteredSwarms(mJumpList.getSelectedIndex(), Integer.parseInt(swarmsPerPagePopup.getItemText(swarmsPerPagePopup.getSelectedIndex())), mFilterPattern, mSortingMetric, fileType.name(), showAllFriendsFiles, selectedFriendID, getSelectedTagPath(), new AsyncCallback<PagedTorrentInfo>() {
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
			...
			public void onSuccess(PagedTorrentInfo result) {
				 ...
				 * Out of order RPC.
				 */
				if (SwarmsBrowser.this.mShowingFriends || SwarmsBrowser.this.mShowingTransfers) {
					return;
				}
				...
				if (mUsingIconView == false) {
					final SwarmsListTable st = new SwarmsListTable(result.swarms, SwarmsBrowser.this);
				...
				if (mFilterPattern != null && mFilterPattern.length() > 0) {
					filterEnabled = true;
					allFilesFiltered.setHTML("<div id=\"" + OneSwarmCss.CSS_NOTHING_SHOWING + "\">" + msg.swarm_browser_no_files_search_friends_HMTL(mFilterPattern) + "</a></div>");

				}
				if (result.total_swarms == 0 && result.by_type_count == 0 && mCurrentFileTypeFilter.equals(FileTypeFilter.All) && selected_shadow == Integer.MIN_VALUE) {
					if (filterEnabled) {
	...
	protected void updateTags(FileTree tags, boolean truncated_tags) {
		...
		if (tags != null) {
			TagEditorDialog.addChildren(root, tags.getChildren(), mEntireUIRoot.getDragController());
		} else {
	...
	private String reverse(TreeItem curr, int depth) {
		if (curr.getParentItem() == null) {
			...
			return reverse(curr.getParentItem(), depth + 1) + curr.getText() + (depth > 0 ? "/" : "");
		}
	}
	...
	public Panel createHeaderButtons() {
		...
		addDownloadButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (url.toLowerCase().startsWith("magnet:") || url.toLowerCase().startsWith("oneswarm:")) {
					...
					OneSwarmRPCClient.getService().getBase64HashesForBase32s(OneSwarmRPCClient.getSessionID(), new String[] { base32hash }, new AsyncCallback<String[]>() {
						public void onFailure(Throwable caught) {
							caught.printStackTrace();
							OneSwarmGWT.log("Error: " + caught.getMessage());
						...
						public void onSuccess(String[] result) {
							...
							for (String s : result) {
								History.newItem(EntireUIRoot.SEARCH_HISTORY_TOKEN + "id:" + s);
							}
						}
					});
				} else {
					OneSwarmRPCClient.getService().downloadTorrent(OneSwarmRPCClient.getSessionID(), url, new AsyncCallback<Integer>() {
						public void onFailure(Throwable caught) {
							caught.printStackTrace();
						}
						...
						public void onSuccess(Integer result) {
							...
							TorrentDownloaderDialog dlg = new TorrentDownloaderDialog(mEntireUIRoot, result);
							dlg.show();
							dlg.setVisible(false);
							dlg.center();
							dlg.setVisible(true);
		...
		tagButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				TagEditorDialog dlg = new TagEditorDialog(mEntireUIRoot, SwarmsBrowser.this.getSelectedSwarms());
				dlg.show();
				dlg.setVisible(false);
				dlg.center();
				dlg.setVisible(true);
		...
		playButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (btext.equals(Strings.get(Strings.SWARM_BROWSER_PLAY)) || btext.equals(msg.swarm_browser_button_download()))
					dispatchSwarmAction(Strings.SWARM_BROWSER_PLAY, getSelectedSwarms());
				else
		...
		moreActions.addChangeListener(new ChangeListener() {
			public void onChange(Widget sender) {
				...
				if (selected.equals(msg.swarm_browser_more_actions_start_swarm()) || selected.equals(msg.swarm_browser_more_actions_stop_swarm())) {
		...
		updateComputerName();
		HTML nameEditAnchor = new HTML("(<a href='#'>" + msg.swarm_browser_change_nick_name() + "</a>)");
		nameEditAnchor.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				ComputerNameDialog dlg = new ComputerNameDialog(computerNameLabel.getText(), true, new ComputerNameDialog.NameSetCallback() {
	...
	public void refreshHeaderButtons() {
			...
			if (anyF2F == false) {
				for (String action : Strings.SWARM_MORE_ACTIONS_SELECTED) {
					...
					if (action.equals(Strings.SWARM_STARTSTOP) == false) {
						...
						newActions.add(selected[0].isStarted() ? msg.swarm_browser_more_actions_stop_swarm() : msg.swarm_browser_more_actions_start_swarm());
					}
				}
			} // if local
			else {
		...
		if (selected != null) {
			...
			{
				canPlay = true;
			} else if (type.equals(FileTypeFilter.Audio) && selected[0].getProgress() * selected[0].getTotalSize() >= 4096 * 100) // lets
			// pretend
			// 4
			// MB
			// is
	...
	public void removeFile(final TorrentInfo inTorrent) {
		...
		if (inTorrent.getProgress() < 1000) {
			...
			if (i != -1) {
				...
				System.err.println("torrent had progress < 1000 but no progress bar. not updated somewhere?" + inTorrent.getTorrentID());
			}
		}
	...
	private class RightClickMenu extends PopupPanel {
		public RightClickMenu() {
			...
			if (anyF2F) {
				MenuItem downloadItem = new MenuItem(msg.swarm_browser_button_download(), true, new Command() {
					public void execute() {
				...
				if (playButton.isEnabled()) {
					MenuItem playItem = new MenuItem(msg.swarm_browser_button_play(), true, new Command() {
						public void execute() {
							dispatchSwarmAction(Strings.SWARM_BROWSER_PLAY, getSelectedSwarms());
							hide();
						}
				...
				MenuItem defaultPlayItem = new MenuItem(msg.swarm_browser_more_actions_default_app(), true, new Command() {
					public void execute() {
						dispatchSwarmAction(Strings.SWARM_DEFAULT_PLAY, getSelectedSwarms());
						hide();
					}
				...
				MenuItem showInFinder = new MenuItem(msg.swarm_browser_more_actions_reveal(), true, new Command() {
					public void execute() {
				...
				if (!anyCompleted) {
					MenuItem startStop = new MenuItem(msg.swarm_browser_right_click_start_stop(), true, new Command() {
						public void execute() {
							dispatchSwarmAction(Strings.SWARM_STARTSTOP, getSelectedSwarms());
							hide();
						}
				...
				MenuItem showDetails = new MenuItem(msg.swarm_browser_more_actions_swarm_details(), true, new Command() {
					public void execute() {
				...
				MenuItem copyLink = new MenuItem(msg.swarm_browser_more_actions_copy_magnet(), true, new Command() {
					public void execute() {
				...
				MenuItem managePerms = new MenuItem(msg.swarm_browser_more_actions_manage_visibility(), true, new Command() {
					public void execute() {
				...
				MenuItem delete = new MenuItem(msg.swarm_browser_button_delete(), true, new Command() {
					public void execute() {
						dispatchSwarmAction(Strings.SWARM_DELETE, getSelectedSwarms());
						hide();
					}
	...
	public void addFileIcon(final TorrentInfo inTorrent, boolean force_show) {
		...
		String disableRightClickCookie = Cookies.getCookie(UISettingsPanel.COOKIE_DISABLE_RIGHT_CLICK);
		if (disableRightClickCookie == null || "1".equals(disableRightClickCookie)) {
			...
			((RightClickablePanel) icon_panel_scratch).addRightClickHandler(new RightClickHandler() {
		...
		videoPreviewImage = new TorrentContainingImage(GWT.getModuleBaseURL() + "image?infohash=" + inTorrent.getTorrentID() + "&type=" + hint, inTorrent);
		...
		Image statusImage = new TorrentContainingImage(inTorrent.getErrorState() == 0 ? ImageConstants.ICON_STARTED : ImageConstants.ICON_WARNING, inTorrent);
		...
		/** 
		 * now always leaving this invisible unless stopped. looks too much like a play icon. confusing.
		 * note, we use the DOM method instead of setVisible since the latter interferes with the selection code's 
		 * use of zIndex, whereas visibility hidden always hides, regardless of zIndex. 
		 */
		...
		if (dragController != null && "0".equals(Cookies.getCookie(UISettingsPanel.COOKIE_DISABLE_DRAG_AND_DROP))) {
	...
	public void update(int count) {
		...
		if (mNextWarningCheck < System.currentTimeMillis()) {
			OneSwarmRPCClient.getService().checkIfWarning(OneSwarmRPCClient.getSessionID(), new AsyncCallback<String[]>() {
				public void onSuccess(String[] result) {
					for (int j = 0; j < 4; j++) {
						if (result[4 + (j * 8)].equals("true")) {
							StoppedUserDialog dlg = new StoppedUserDialog(mEntireUIRoot, result[5 + (j * 8)], result[6 + (j * 8)], result[7 + (j * 8)]);
							dlg.show();
							dlg.setVisible(false);
							dlg.center();
							dlg.setVisible(true);
						} else if (result[(j * 8)].equals("true")) {
							WarnUserDialog dlg = new WarnUserDialog(mEntireUIRoot, result[1 + (j * 8)], result[2 + (j * 8)], result[3 + (j * 8)]);
							dlg.show();
							dlg.setVisible(false);
							dlg.center();
							dlg.setVisible(true);
		...
		if (mNextErrorPoll < System.currentTimeMillis()) {
			...
			OneSwarmRPCClient.getService().getBackendErrors(OneSwarmRPCClient.getSessionID(), new AsyncCallback<ArrayList<BackendErrorReport>>() {
				public void onFailure(Throwable caught) {
					caught.printStackTrace();
				}
	...
	private void lightweight_refresh_check() {
		OneSwarmRPCClient.getService().getDownloadManagersCount(OneSwarmRPCClient.getSessionID(), new AsyncCallback<Integer>() {
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
			...
			public void onSuccess(Integer result) {
				if (result.intValue() != mPreviousFullRefreshTotalSwarmsCount) {
					System.out.println("download manager count doesn't match mPreviousFullRefreshTotalSwarmsCount, refreshing (" + result + " / " + mPreviousFullRefreshTotalSwarmsCount);
					if (mShowingTransfers == false || mShowingFriends) {
						SwarmsBrowser.this.refreshActive(false);
					}
				} else {
					// System.out.println("lightweight refresh: " + result);
				}
			}
		});
	}
	...
	private void pagedSwarmRefresh() {
		...
		for (int i = 0; i < filesFlowPanel.getWidgetCount(); i++) {
			if (filesFlowPanel.getWidget(i) instanceof PreviewPanel) {
				...
				AbsolutePanel fnameAbsolute = (AbsolutePanel) fnamePanel.getWidget(1);
				MaxHeightWithTorrentInfoLabel fnameLabel = (MaxHeightWithTorrentInfoLabel) fnameAbsolute.getWidget(0);
			...
			public void onSuccess(TorrentInfo[] result) {
				...
				for (TorrentInfo t : result) {
					...
					if (id_to_torrent.get(t.getTorrentID()) != null) {
						...
						id_to_torrent.get(t.getTorrentID()).setDownloadRate(t.getDownloadRate());
						id_to_torrent.get(t.getTorrentID()).setDownloaded(t.getDownloaded());
						
						id_to_torrent.get(t.getTorrentID()).setSeeders(t.getSeeders());
						id_to_torrent.get(t.getTorrentID()).setLeechers(t.getLeechers());

						id_to_torrent.get(t.getTorrentID()).setErrorState(t.getErrorState());
						
						/**
						 * In case the play button now needs enabled
						 */
						if (mSelectedSwarms.size() == 1) {
							if (mSelectedSwarms.get(0).getTorrentID().equals(t.getTorrentID()))
								refreshHeaderButtons();
						}
					}
				}
	...
	private void gotRefreshOfTransferring(TorrentInfo[] swarms) {
		...
		for (int swarmsIndex = 0; swarmsIndex < swarms.length; swarmsIndex++) {
			...
			if (i > -1) {
				 ...
				 * wrap this?
				 */
				final VerticalPanel fnamePanel = (VerticalPanel) torrent_to_panel.get(info).getWidget(); // this
				// is
				// icon_with_name
				// verticalpanel
				AbsolutePanel fnameAbsolute = (AbsolutePanel) fnamePanel.getWidget(1);
				...
				if (latest != null) {
					...
					if (latest.getProgress() == 1000) {
						 ...
						 * so give that a shot
						 */
						Image previewImage = (Image) ((AbsolutePanel) fnamePanel.getWidget(0)).getWidget(0);
		...
		OneSwarmRPCClient.getService().getIntegerParameterValue(OneSwarmRPCClient.getSessionID(), "OneSwarm.ui.double.click", new AsyncCallback<Integer>() {
			...
			public void onSuccess(Integer result) {
				...
				if (result >= 0 && result < 3) {
					...
					System.out.println("doubleclick action str: " + mDoubleClickAction.getAction());
				}
			}
		});
	...
	public void doubleClick() {
		...
		if (OneSwarmGWT.isRemoteAccess() == false) {
			if (mDoubleClickAction.getAction().equals(Strings.SWARM_BROWSER_PLAY) && playButton.isEnabled() == false) {
				dispatchSwarmAction(Strings.SWARM_DEFAULT_PLAY, getSelectedSwarms());
			} else {
