...
public class KeywordFilterSettingsPanel extends SettingsPanel implements ClickHandler {
	...
	public KeywordFilterSettingsPanel() {
		...
		OneSwarmRPCClient.getService().getStringListParameterValue(OneSwarmRPCClient.getSessionID(), "oneswarm.search.filter.keywords", new AsyncCallback<ArrayList<String>>(){
	...
	public void onClick(ClickEvent event) {
		if( event.getSource().equals(addButton) ) {
			...
			if( neu != null ) {
				if( neu.length() >= 3 ) {
					currentKeywords.add(neu + " (" + msg.add_friends_invite_create_delivery_manual() + ")");
					refreshUI();
				} else {
	...
	public void sync() {
		...
		OneSwarmRPCClient.getService().setStringListParameterValue(OneSwarmRPCClient.getSessionID(), "oneswarm.search.filter.keywords", params, new AsyncCallback<Void>(){
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
			}
