...
public class FriendsDetailsTable extends ScrollTable {
	...
	private static final HeaderWithWidth[] COLUMNS = new HeaderWithWidth[] { new HeaderWithWidth("", 15),
	...
	private void setupHeader() {
		for (int i = 0; i < COLUMNS.length; i++) {
			...
			if (COLUMNS[i].center) {
				mHeader.getCellFormatter().setHorizontalAlignment(0, i, HorizontalPanel.ALIGN_CENTER);
			}
		}
	}
	...
	public void forceConnect() {
		...
		OneSwarmRPCClient.getService().connectToFriends(OneSwarmRPCClient.getSessionID(), this.getSelectedFriends().toArray(new FriendInfoLite[0]), new AsyncCallback<Void>() {
	...
	public void blockClicked() {
		...
		OneSwarmRPCClient.getService().setFriendsSettings(OneSwarmRPCClient.getSessionID(), selected.toArray(new FriendInfoLite[0]), new AsyncCallback<Void>() {
			public void onFailure(Throwable caught) {
	...
	public void removeClicked() {
		...
		if (Window.confirm(prompt)) {
			...
			OneSwarmRPCClient.getService().deleteFriends(OneSwarmRPCClient.getSessionID(), selectedArr, new AsyncCallback<Void>() {
				public void onFailure(Throwable caught) {
	...
	public void refresh() {
		OneSwarmRPCClient.getService().getNewFriendsCountsFromAutoCheck(OneSwarmRPCClient.getSessionID(), new AsyncCallback<HashMap<String, Integer>>() {
		...
		OneSwarmRPCClient.getService().getFriends(OneSwarmRPCClient.getSessionID(), 0, true, true, new AsyncCallback<FriendList>() {
			public void onSuccess(FriendList result) {
				...
				/**
				 * If there are no friends we won't be showing the table anyway
				 */
...
//					 System.out.println("INSIDE FOR LOOP: " + i);
					...
					if (mShowBlocked || ((Label) (frienddata.get("Deleted"))).getText().equals("false")) {
						SortableNamePanel sortableNamePanel = (SortableNamePanel) frienddata.get("Name");
