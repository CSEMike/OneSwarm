...
public class EntireUIRoot extends DockPanel {
	...
	ValueChangeHandler<String> historyChangeListener = new ValueChangeHandler<String>() {
		public void onValueChange(ValueChangeEvent<String> event) {
			...
			if (historyToken.equals(NavigationSidePanel.HYPERLINK_LABEL_TRANSFERS)) {
				...
				CommunityRecord selectedServer = navSidePanel.getCommunityServersPanel().getSelectedServer();
				if (selectedServer == null) {
					...
					final Frame frame = new Frame(base + "/?osclient=" + URL.encode(mFooter.getVersion()));
					frame.setWidth("98%");
		...
		(new Timer() {
			public void run() {
				...
				if (getPlayingVideo() == false && (mLastAction + 60 * 1000) < now) {
					...
					System.out.println("skipped f5 refresh due to playing or recent action " + (new Date()));
				}
			}
	...
	public void friendFilterChanged() {
		FriendInfoLite selectedFriend = navSidePanel.getFriendPanel().getFriendListPanel().getSelectedFriend();
	...
	public void startChat(final FriendInfoLite selectedFriend) {
		if (selectedFriend != null) {
			if (selectedFriend.isSupportsChat() == false && selectedFriend.getStatus() == FriendInfoLite.STATUS_ONLINE) {
				Window.alert("This user's OneSwarm client does not support chat (or has disabled it).");
				return;
			}

			if (selectedFriend.isAllowChat() == false && selectedFriend.getStatus() == FriendInfoLite.STATUS_ONLINE) {
				Window.alert("You have disabled chat for this friend.");
				return;
			}
		}

		final FriendInfoLite[] allFriends = navSidePanel.getFriendPanel().getFriendListPanel().getAllFriends();
		...
		if (openNewWindow) {
			ChatDialog dlg = new ChatDialog(allFriends, selectedFriend != null ? selectedFriend.getPublicKey() : null, this);
		}
	}
