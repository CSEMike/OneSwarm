...
public class AddTorrentManager implements RequiresShutdown {
    ...
    public int downloadTorrent(int friendConnection, int channelId, String torrentId, int lengthHint) {
        ...
        coreInterface.getF2FInterface().getMetaInfo(friendConnection, channelId, torrentId,
                lengthHint, new PluginCallback<byte[]>() {
                    ...
                    public void requestCompleted(byte[] data) {
                        try {
                            ...
                            /**
                             * We've modified the TOTorrentImpl to preserve the
                             * required OneSwarm attributes while dumping the
                             * rest (this include no text search, tags, album,
                             * etc.)
                             */
                            TOTorrent torrent = TorrentUtils
                                    .readFromBEncodedInputStream(new ByteArrayInputStream(data));
    ...
    public int downloadTorrent(String url) {
        ...
        try {
            ...
            TorrentDownloader torrentDownloader = TorrentDownloaderFactory.create(
                    new TorrentDownloaderCallBackInterface() {

                        public void TorrentDownloaderEvent(int state, TorrentDownloader inf) {
                            try {
                                logger.finest("got torrent download event: state=" + state
                                        + " done=" + inf.getPercentDone() + " URL='" + inf.getURL()
                                        + "'");
                                try {
        ...
        if (inf != null && inf.getDownloadState() == TorrentDownloader.STATE_FINISHED) {
            ...
            try {
                ...
                for (int i = 0; i < files.length; i++) {
                    ...
                    try {
                        fList[i] = new FileListLite(new String(Base64.encode(torrent.getHash())),
                                new String(torrent.getName(), "UTF-8"),
                                torrentFile.getRelativePath(), torrentFile.getLength(),
                                files.length, new Date().getTime(), 1, false, false);
                    } catch (UnsupportedEncodingException e) {
                        System.err.println(e);
                        logger.severe(e.toString());
                        fList[i] = new FileListLite(new String(Base64.encode(torrent.getHash())),
                                new String(torrent.getName()), torrentFile.getRelativePath(),
                                torrentFile.getLength(), files.length, new Date().getTime(), 1,
                                false, false);
                    }
                    if (limitedVisibility) {
                    ...
                    if (flf.getEd2kHash() != null) {
                        fList[i].setEd2kHash(new String(ShareManagerTools.base16Encode(flf
                                .getEd2kHash())));
                    }
                }
                return fList;
    ...
    public boolean addTorrent(int torrentDownloaderHash, final FileListLite[] selectedFiles,
            final List<PermissionsGroup> inPerms, String path, final boolean noStream) {
        ...
        try {
            ...
            while (!quit && inf.getDownloadState() != TorrentDownloader.STATE_FINISHED) {
                logger.finer("waiting for torrent file to download: downloaded="
                        + inf.getTotalRead() + " (" + inf.getPercentDone() + "%)");
                Thread.sleep(100);
            }
            if (!quit) {
                File f = inf.getFile();
                ...
                for (PermissionsGroup g : inPerms) {
                    try {
                        ...
                        Debug.out("problem converting PermissionsGroup to GroupBean: "
                                + g.getName());
                        e.printStackTrace();
                    }
                }

                return ShareManagerTools.addDownload(selectedFilesSet, converted_groups, path,
                        noStream, f, torrent) != null;
            }
