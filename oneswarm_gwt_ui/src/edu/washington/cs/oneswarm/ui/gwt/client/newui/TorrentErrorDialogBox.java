...
//	static final int HEIGHT = 250;
	...
	public TorrentErrorDialogBox( final TorrentInfo inTorrent, EntireUIRoot inRoot )
	{
		...
		setText(msg.torrent_error_dialog_warning() + ": " + StringTools.truncate(inTorrent.getName(), 32, true));
...
//		status_and_button.setWidth("100%");
	...
	public void onClick(ClickEvent event) {
		...
		if( source.equals(dismissButton) ) {
			...
			OneSwarmRPCClient.getService().fixPermissions( OneSwarmRPCClient.getSessionID(), mTorrent, false, new AsyncCallback<Void>(){
			...
			OneSwarmRPCClient.getService().fixPermissions( OneSwarmRPCClient.getSessionID(), mTorrent, true, new AsyncCallback<Void>(){
				public void onFailure(Throwable caught) {
					if( caught instanceof OneSwarmException ) {
						new ReportableErrorDialogBox((OneSwarmException)caught, false);
					} else {
