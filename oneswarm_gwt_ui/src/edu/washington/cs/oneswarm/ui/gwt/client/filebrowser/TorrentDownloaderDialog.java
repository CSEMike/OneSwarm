...
public class TorrentDownloaderDialog extends OneSwarmDialogBox implements Updateable {
	 ...
	 */
	public TorrentDownloaderDialog(EntireUIRoot inRoot, OneSwarmDialogBox toCloseIfDownload, int friendId, FileListLite selectedFile, boolean filesAvailable) {
		this(inRoot, toCloseIfDownload, friendId, 0, 0, selectedFile, filesAvailable, null, null);
	}

	public TorrentDownloaderDialog(EntireUIRoot inRoot, OneSwarmDialogBox toCloseIfDownload, int friendId, int channelId, int metaInfoLengthHint, FileListLite selectFile, boolean filesAvailable, String[] inFromFriends, Long[] inDelays) {
		super(false, false, true);

		this.setText(msg.torrent_download_details() + ": " + StringTools.truncate(selectFile.getCollectionName(), 45, true));
	...
	private ScrollTable createSourcesTable() {
		...
		for (int i = 0; i < mFromFriends.length; i++) {
			...
			data.setText(i, 1, (mFriendsDelays == null ? "Unknown" : mFriendsDelays[i].toString() + " ms"));
		}
	...
	private void startMetaInfoDownload() {
		...
		service.downloadTorrent(session, friendId, channelId, selectedFile.getCollectionId(), metaInfoLengthHint, cb);
		OneSwarmGWT.addToUpdateTask(this);
	}
	...
	private void startTorrentDownload() {
		...
		addTorrentService.addTorrent(session, torrentDownloadId, filesToDownload.toArray(new FileListLite[0]), permGroups, mCustomSavePath, noStream, new AsyncCallback<Boolean>() {
			...
			public void onSuccess(Boolean result) {
				...
				if (res.booleanValue()) {
					// OneSwarmGWT.log("Torrent added successfully, attempting
					// front-end refresh");
					mRoot.refreshSwarms();
	...
	public void update(int count) {
		...
		if (torrentDownloadId != -1) {
			final String session = OneSwarmRPCClient.getSessionID();
			OneSwarmUIServiceAsync service = OneSwarmRPCClient.getService();
			service.getTorrentDownloadProgress(session, torrentDownloadId, new AsyncCallback<Integer>() {
	...
	private void updateCollection(final int friendId, boolean filesAvailable) {
		...
		AsyncCallback<FileListLite[]> cb = new AsyncCallback<FileListLite[]>() {
			...
			public void onSuccess(final FileListLite[] result) {
				...
				if (mFromURL) {
					OneSwarmRPCClient.getService().getTorrentName(OneSwarmRPCClient.getSessionID(), torrentDownloadId, new AsyncCallback<String>() {
						public void onFailure(Throwable caught) {
							caught.printStackTrace();
						}
						...
						public void onSuccess(String name) {
							...
							TorrentDownloaderDialog.this.setText(msg.torrent_download_details() + ": " + name);
							...
							/*
							 * init the permissions, this was from a url so
					...
					if (result.length > 0 && result[0].isOneSwarmNoShare()) {
						 ...
						 */
						OneSwarmRPCClient.getService().getFriends(OneSwarmRPCClient.getSessionID(), 0, true, true, new AsyncCallback<FriendList>() {
							public void onFailure(Throwable caught) {
							}
		...
		if (filesAvailable) {
			if (mFromURL == false) {
				service.getFileList(session, friendId, "id:" + selectedFile.getCollectionId(), 0, 1000, 24 * 60 * 60 * 1000, cb);
			} else {
	...
	class FilesPanel extends VerticalPanel {
		...
		private void update() {
			...
			for (int i = 0; i < collection.length; i++) {
				 ...
				 */
				if (checkEverything || selectedFile == null || f.getFileName().equals(selectedFile.getFileName())) {
					b.setChecked(true);
				} else {
					b.setChecked(false);
				}
	...
	private class InfoPanel extends VerticalPanel {
		...
		public InfoPanel(FileListLite fileListLite, String[] sources) {
					...
					OneSwarmRPCClient.getService().selectFileOrDirectory(OneSwarmRPCClient.getSessionID(), true, new AsyncCallback<String>() {
						...
						public void onSuccess(String result) {
							...
							if (result != null) {
								...
								locationLabel.setText(StringTools.truncate(result, 50, true));
							}
						}
					});
				}
			...
			if (Cookies.getCookie(IN_ORDER_DOWNLOAD_COOKIE) != null) {
				 ...
				 * other
				 */
				if (selectedFile != null && OneSwarmConstants.InOrderType.getType(selectedFile.getFileName()) != null) {
			...
			streamCheck.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent sender) {
					...
					Cookies.setCookie(IN_ORDER_DOWNLOAD_COOKIE, downloadInOrder + "", OneSwarmConstants.TEN_YEARS_FROM_NOW);
				}
			});

			streamCheckPanel.add(streamCheck);
			HelpButton help = new HelpButton("Streaming downloads files in-order so you can play them during transfer. But, downloading out-of-order may improve download speeds.");
	...
	class PermissionsPanel extends VerticalPanel {
				...
				public void onSuccess(ArrayList<PermissionsGroup> result) {
					...
					if (friendOnly != null) {
						...
						for (PermissionsGroup permissionsGroup : result) {
							if (permissionsGroup.isUserGroup() && permissionsGroup.containsUser(friendPublicKey)) {
								initial_groups.add(permissionsGroup);
								break;
							}
						}
					} else {
						initial_groups.add(new PermissionsGroup(PermissionsGroup.ALL_FRIENDS));
						if (includePublic) {
							initial_groups.add(new PermissionsGroup(PermissionsGroup.PUBLIC_INTERNET));
						}
					}
		...
		private void initThisUI(final List<PermissionsGroup> all_groups) {
			...
			sharing_with_groups = new MembershipList<PermissionsGroup>("Will share with:", false, all_groups, all_sub_shared, true);
