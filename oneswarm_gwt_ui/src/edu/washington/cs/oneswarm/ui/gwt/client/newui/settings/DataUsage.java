...
public class DataUsage extends SettingsPanel implements Updateable {
    ...
    DataUsage() {
        ...
        OneSwarmRPCClient.getService().getLimits(OneSwarmRPCClient.getSessionID(),
                new AsyncCallback<HashMap<String, String>>() {
                    ...
                    public void onSuccess(HashMap<String, String> result) {
                        if (!(result.get(Strings.SIDEBAR_DAILYLIMIT).equals("")
                                && result.get(Strings.SIDEBAR_WEEKLYLIMIT).equals("")
                                && result.get(Strings.SIDEBAR_MONTHLYLIMIT).equals("") && result
                                .get(Strings.SIDEBAR_YEARLYLIMIT).equals(""))) {
    ...
    public void sync() {
        ...
        OneSwarmRPCClient.getService().setLimits(OneSwarmRPCClient.getSessionID(),
                dailylimit.getText(), weeklylimit.getText(), monthlylimit.getText(),
                yearlylimit.getText(), new AsyncCallback<String>() {
                    public void onFailure(Throwable caught) {
                        caught.printStackTrace();
                    }

                    public void onSuccess(String result) {
                        System.out.println("Updated Limits to following: day: "
                                + dailylimit.getText() + " week: " + weeklylimit.getText()
                                + " month: " + monthlylimit.getText() + " year: "
                                + yearlylimit.getText());
                    }
                });
    }
    ...
    public void update(int count) {
        if (mNextUpdate < System.currentTimeMillis()) {
            ...
            OneSwarmRPCClient.getService().getCounts(OneSwarmRPCClient.getSessionID(),
                    new AsyncCallback<HashMap<String, String>>() {
                        public void onFailure(Throwable caught) {
                            caught.printStackTrace();
                        ...
                        public void onSuccess(HashMap<String, String> result) {
                            ...
                            if (result != null) {
                                if (!(result.get(Strings.SIDEBAR_DAYCOUNT).equals("0")
                                        && result.get(Strings.SIDEBAR_WEEKCOUNT).equals("0")
                                        && result.get(Strings.SIDEBAR_MONTHCOUNT).equals("0") && result
                                        .get(Strings.SIDEBAR_YEARCOUNT).equals("0"))) {
                                    thisday.setText(": "
                                            + StringTools.formatRate(result
                                                    .get(Strings.SIDEBAR_DAYCOUNT)));
                                    thisweek.setText(": "
                                            + StringTools.formatRate(result
                                                    .get(Strings.SIDEBAR_WEEKCOUNT)));
                                    thismonth.setText(": "
                                            + StringTools.formatRate(result
                                                    .get(Strings.SIDEBAR_MONTHCOUNT)));
                                    thisyear.setText(": "
                                            + StringTools.formatRate(result
                                                    .get(Strings.SIDEBAR_YEARCOUNT)));
                                }
                            }
