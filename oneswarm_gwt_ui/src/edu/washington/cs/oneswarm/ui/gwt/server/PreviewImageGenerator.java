...
public class PreviewImageGenerator extends javax.servlet.http.HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			logger.finest("serving image: " + (imageFile != null ? imageFile.getAbsolutePath() : "(null)") + " / " + hash);
			if (imageFile != null) {
				...
				response.setHeader(HttpHeaders.CACHE_CONTROL, "max-age=" + secondsToCache + ", private");
	...
	private void processArbitraryPathRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			response.setContentType("image/png");
			previewImageForArbitraryPath(URLDecoder.decode(request.getParameter("path"), "UTF-8"), scale, response.getOutputStream());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void previewImageForArbitraryPath(String path, int scale, OutputStream outStream) throws IOException {
		FileInputStream fis = new FileInputStream(path);
	...
	public File getImageFile(String inInfohash) {
		...
		try {
			...
			try {
				return FFMpegAsyncOperationManager.getInstance().getPreviewImage(torrent.getHash(), fileInfo.getFile(), PREVIEW_MAX_WAIT_TIME, TimeUnit.SECONDS);
